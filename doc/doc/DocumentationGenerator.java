...
public class DocumentationGenerator {
	...
	private void getRootDoc() {
		try {
			getRootDoc(new File(ParameterService.getRapidMinerHome(), "src" + File.separator), "base.operators.operator");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static void main(String[] argv) throws IOException {
		if (argv.length == 1) {
			...
			LogService.getGlobal().log("usage: java base.operators.doc.DocumentationGenerator or" + Tools.getLineSeparator()
					+ "       java base.operators.doc.DocumentationGenerator operatordesc srcdir subpackages outputfile [generate subgroups (true/false)]",
					LogService.WARNING);
		}
	}
}
