...
import base.operators.operator.learner.functions.neuralnet.ActivationFunction;
import base.operators.operator.learner.functions.neuralnet.InnerNode;
import base.operators.operator.learner.functions.neuralnet.InputNode;
import base.operators.operator.learner.functions.neuralnet.LinearFunction;
import base.operators.operator.learner.functions.neuralnet.Node;
import base.operators.operator.learner.functions.neuralnet.OutputNode;
import base.operators.operator.learner.functions.neuralnet.SigmoidFunction;
	...
	public void train(ExampleSet exampleSet, List<String[]> hiddenLayers, int maxCycles, double maxError,
		...
		for (int cycle = 0; cycle < maxCycles; cycle++) {
			...
			if (Double.isInfinite(error) || Double.isNaN(error)) {
				if (base.operators.tools.Tools.isLessEqual(learningRate, 0.0d)) // should hardly happen
				{
		...
		for (InnerNode innerNode : innerNodes) {
			...
			if (layerIndex != Node.OUTPUT) {
				if (lastLayerIndex == -99 || lastLayerIndex != layerIndex) {
					if (!first) {
						result.append(base.operators.tools.Tools.getLineSeparators(2));
					}
					first = false;
					String layerName = "Hidden " + (layerIndex + 1);
					result.append(layerName + base.operators.tools.Tools.getLineSeparator());
					for (int t = 0; t < layerName.length(); t++) {
						result.append("=");
					}
					lastLayerIndex = layerIndex;
					result.append(base.operators.tools.Tools.getLineSeparator());
				}
				// node name and type
				String nodeName = innerNode.getNodeName() + " (" + innerNode.getActivationFunction().getTypeName() + ")";
				result.append(base.operators.tools.Tools.getLineSeparator() + nodeName + base.operators.tools.Tools.getLineSeparator());
				...
				for (int i = 0; i < inputNodes.length; i++) {
					result.append(inputNodes[i].getNodeName() + ": " + base.operators.tools.Tools.formatNumber(weights[i + 1])
							+ base.operators.tools.Tools.getLineSeparator());
				}
				// threshold weight
				result.append("Threshold: " + base.operators.tools.Tools.formatNumber(weights[0]) + base.operators.tools.Tools.getLineSeparator());
			}
		}
