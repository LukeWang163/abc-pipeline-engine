...
import base.operators.utils.DataTypeUtil;
...
public class HDFSUtil {
	...
	private final static Log log = LogFactory.getLog(base.operators.utils.HDFSUtil.class);
	...
	public static String getZip(String src, String tempDir) {
		try {
			...
			fs.close();
			return base.operators.utils.FileUtil.zip(tempDir + "/" + src.substring(src.lastIndexOf("/") + 1));
		} catch (IOException e) {
			if (log.isDebugEnabled()) {
	...
	public static String getZip(String[] paths, String tempDir) {
		try {
			...
			fs.close();
			return base.operators.utils.FileUtil.zip(tempDir);
		} catch (IOException e) {
			if (log.isDebugEnabled()) {
				log.debug("Compressed file throw exception:", e);
			}
	...
	public static void copy(String src,String dest) throws IOException{
		FileSystem fs=getFileSystem();
		// 不删除src源文件
		FileUtil.copy(fs, new Path(src), fs, new Path(dest), false, fs.getConf());
	}
