...
*/
package base.operators.generator;
...
public abstract class FeatureGenerator {
	...
	public static FeatureGenerator createGeneratorForFunction(String functionName) {
		...
		try {
			...
			LogService.getRoot().log(Level.SEVERE, "base.operators.generator.FeatureGenerator.instantiating_error",
					genClass.getName());
			return null;
		}
	...
	public static List<Attribute> generateAll(ExampleTable exampleTable, Collection<FeatureGenerator> generatorList)
		...
		LogService.getRoot().log(Level.FINE, "base.operators.generator.FeatureGenerator.starting_feature_generation",
				generatorList.size());
		...
		LogService.getRoot().log(Level.FINE, "base.operators.generator.FeatureGenerator.generator_list", generatorList);
		LogService.getRoot().log(Level.FINE,
				"base.operators.generator.FeatureGenerator.input_has_feature_count_and_example_count",
		...
		LogService.getRoot().log(Level.FINE, "base.operators.generator.FeatureGenerator.finished_feature_generation");
		LogService.getRoot().log(Level.FINE,
				"base.operators.generator.FeatureGenerator.generated_set_has_feature_count_and_example_count",
				new Object[] { exampleTable.getAttributeCount(), exampleTable.size() });
	...
	private static List<Attribute> newAttributes(FeatureGenerator[] generators, ExampleTable exampleTable) {
		...
		for (int i = 0; i < generators.length; i++) {
			...
			if (!generators[i].argumentsOk(exampleTable)) {
				LogService.getRoot().log(Level.WARNING, "base.operators.generator.FeatureGenerator.wrong_argument_types",
						generators[i]);
			}
		}
		return newAttributeList;
