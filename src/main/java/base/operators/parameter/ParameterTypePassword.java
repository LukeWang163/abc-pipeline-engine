...
public class ParameterTypePassword extends ParameterTypeString {
		...
		if (CipherTools.isKeyAvailable()) {
			try {
				...
				LogService.getRoot().log(Level.SEVERE,
				        "base.operators.parameter.ParameterTypePassword.encrypting_password_error");
				return value;
			}
		} else {
			return value;
		...
		if (CipherTools.isKeyAvailable()) {
			try {
				...
				LogService.getRoot().log(Level.FINE,
				        "base.operators.parameter.ParameterTypePassword.password_looks_like_unencrypted_plain_text");
			}
		}
		return value;
	}
	...
	public static void main(String[] args) {
		try {
			System.out.println(CipherTools.encrypt("1a2b3f4g"));
		} catch (CipherException e) {
			e.printStackTrace();
		}
	}
