...
import base.operators.operator.ports.MetaDataChangeListener;
...
import base.operators.external.alphanum.AlphanumComparator;
...
public class ParameterTypeAttribute extends ParameterTypeString {
	...
	public void getDefinitionToXML(Element typeElement) {
		//super.writeDefinitionToXML(typeElement);
		...
		Element allowedTypesElement = XMLTools.addTag(typeElement, ELEMENT_ALLOWED_TYPES);
		for (int allowedValueType : allowedValueTypes) {
			Element element = XMLTools.addTag(allowedTypesElement, ELEMENT_ALLOWED_TYPE);
			element.setAttribute("Value", allowedValueType+"");
		}
	}
}
