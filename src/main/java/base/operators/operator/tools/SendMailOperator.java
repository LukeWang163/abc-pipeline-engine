...
import base.operators.tools.MailUtilities;
import base.operators.tools.ParameterService;
import base.operators.operator.MailNotSentException;
	...
	protected void performAdditionalChecks() {
		...
		if (method.equals(RapidMiner.PROPERTY_RAPIDMINER_TOOLS_MAIL_METHOD_VALUES[0])) { // sendmail
			...
			if (command == null || command.equals("")) {
				addError(new SimpleProcessSetupError(ProcessSetupError.Severity.WARNING, getPortOwner(), "no_send_mail_command"));
			}
			...
			if (user == null || user.equals("")) {
				addError(new SimpleProcessSetupError(ProcessSetupError.Severity.WARNING, getPortOwner(), "no_smtp_mail_user_set"));
			}

			String passwd = ParameterService.getParameterValue(RapidMiner.PROPERTY_RAPIDMINER_TOOLS_SMTP_PASSWD);
			if (passwd == null || passwd.equals("")) {
				addError(new SimpleProcessSetupError(ProcessSetupError.Severity.WARNING, getPortOwner(), "no_smtp_mail_passwd_set"));
			}

			String host = ParameterService.getParameterValue(RapidMiner.PROPERTY_RAPIDMINER_TOOLS_SMTP_HOST);
			if (host == null || host.equals("")) {
				addError(new SimpleProcessSetupError(ProcessSetupError.Severity.WARNING, getPortOwner(), "no_smtp_mail_host_set"));
			}

			String port = ParameterService.getParameterValue(RapidMiner.PROPERTY_RAPIDMINER_TOOLS_SMTP_PORT);
			if (port == null || port.equals("")) {
				addError(new SimpleProcessSetupError(ProcessSetupError.Severity.WARNING, getPortOwner(), "no_smtp_mail_port_set"));
			}
		}
