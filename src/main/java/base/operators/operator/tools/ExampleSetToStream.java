...
import base.operators.example.table.DoubleSparseArrayDataRow;
import base.operators.example.table.NominalMapping;
import base.operators.example.table.PolynominalMapping;
import base.operators.example.table.SparseDataRow;
...
public class ExampleSetToStream {
	...
	public ExampleSetToStream(final int version) {
		...
		if (version != CURRENT_VERSION) {
			LogService.getRoot().log(Level.FINE,
					"base.operators.operator.tools.ExampleSetToStream.using_deprecated_version", version);
		}
	}
	...
	public void writeAnnotations(final DataOutput out, final Annotations annotations) throws IOException {
		if (version < VERSION_3) {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.operator.tools.ExampleSetToStream.ignoring_annotations",
					version);
		} else {
			if (annotations == null) {
				out.writeInt(0);
