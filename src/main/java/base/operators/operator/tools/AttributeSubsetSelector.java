...
import base.operators.example.set.Condition;
...
import base.operators.operator.ports.quickfix.AbstractQuickFix;
...
import base.operators.operator.preprocessing.filter.attributes.AttributeFilterCondition;
import base.operators.operator.preprocessing.filter.attributes.BlockTypeAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.NoMissingValuesAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.NumericValueAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.RegexpAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.SingleAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.SubsetAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.TransparentAttributeFilter;
import base.operators.operator.preprocessing.filter.attributes.ValueTypeAttributeFilter;
			...
			if (errorOnMissing) {
				if (condition instanceof SingleAttributeFilter) {
					if (resultSet.getAllAttributes().size() != 1) {
						inPort.addError(new SimpleMetaDataError(ProcessSetupError.Severity.ERROR, inPort, "missing_attribute",
					...
					if (!shouldBeFound.isEmpty()) {
						...
						if (!shouldBeFound.isEmpty()) {
							...
							for (String attName : shouldBeFound) {
								inPort.addError(
										new SimpleMetaDataError(ProcessSetupError.Severity.ERROR, inPort, "missing_attribute", attName));
							}
						}
					}
				}
			...
			if (condition.isNeedingScan()) {
				...
				while (r.hasNext()) {
					...
					for (Example example : exampleSet) {
						...
						if (result != AttributeFilterCondition.ScanResult.UNCHECKED) {
					...
					if (condition.isNeedingFullScan()) {
						...
						// removed (=keep)
						if (afterScanResult == AttributeFilterCondition.ScanResult.UNCHECKED) {
			...
			public void check(MetaData metaData) {
				if (metaData instanceof ExampleSetMetaData) {
					...
					if (subsetMetaData.getAllAttributes().isEmpty()) {
						...
						SimpleMetaDataError error = new SimpleMetaDataError(ProcessSetupError.Severity.WARNING, inPort,
