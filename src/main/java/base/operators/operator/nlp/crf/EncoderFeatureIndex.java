...
public class EncoderFeatureIndex extends FeatureIndex
{
    ...
    /**
     * 读取特征模板文件2(前端传来，为一行，方便起见，直接转为以前的格式，这样后面的函数无需修改)
     *
     * @param temp
     * @return
     */
    private boolean openTemplate2(String temp)
    {
     ...
     * @return
     */
    private boolean openTagSet(List<List<String> > text)
    {
        int max_size = 0;
        ...
        for(List<String> list : text) {
            for (String line : list) {
                ...
                if (max_size != cols.length) {
                    System.out.println("inconsistent column size: " + max_size +
                            " " + cols.length + " " + line);
                    continue;
                }
                xsize_ = cols.length - 1;
    ...
    public boolean open(String template, List text)
    {
        checkMaxXsize_ = true;
        return openTemplate2(template) && openTagSet(text);
    }
    ...
    public boolean save(String filename, boolean textModelFile)
    {
        try
        {
                ...
                MutableDoubleArrayTrieInteger dic = dic_;
                oos.writeObject(dic);
