...
public class Model implements Serializable{
	...
	public int burnStep; //burn-in steps
		...
		try{
			...
			for (int i = 0; i < M; i++){
				for (int j = 0; j < K; j++){
					...
					writer.write(decimal(theta[i][j], 4) + " ");
				}
				writer.write("\n");
			}
			writer.close();
	...
	public boolean saveModelPhi(String filename){
		try {
			...
			for (int i = 0; i < K; i++){
				for (int j = 0; j < V; j++){
					writer.write(decimal(phi[i][j], 4) + " ");
				}
				writer.write("\n");
			}
			writer.close();
	...
	/**
	 * Init parameters for train
	 *  训练时使用
	 */
	@SuppressWarnings("unchecked")
	public boolean initNewModel(LDAOption option, List<String> text){
		if (!init(option))
			return false;
		...
		data = LDADataset.buildDataSet(text);
		if (data == null){
	...
	/**
	 * read other file to get parameters(模型参数)
	 */
	protected boolean readOthersFile(String otherFile){
		...
		try {
			...
			while((line = reader.readLine()) != null){
				...
				else if (optstr.equalsIgnoreCase("liters")){
	...
	//保留小数点位数，保留n位
	private static double decimal(double d, int n) {
