 ...
 * description:
 */
public class DocSimilarityOperator extends Operator {
    private InputPort exampleSetInput = getInputPorts().createPort("example set");
    private OutputPort exampleSetOutput = getOutputPorts().createPort("example set");

    public static final String ORIGINAL_DOC_ATTRIBUTE_NAME = "original_doc_attribute_name";
    public static final String MAP_DOC_ATTRIBUTE_NAME = "map_doc_attribute_name";
    ...
    public static String[] METHODS = {"levenshtein_sim","damerau_levenstein_sim","lcs_sim","ssk","cosine","simhash_hamming_sim"};

    public DocSimilarityOperator(OperatorDescription description){
        super(description);
        exampleSetInput.addPrecondition(
                new AttributeSetPrecondition(exampleSetInput, AttributeSetPrecondition.getAttributesByParameter(
                        this, ORIGINAL_DOC_ATTRIBUTE_NAME,MAP_DOC_ATTRIBUTE_NAME)));

    }

    @Override
    public void doWork() throws OperatorException {
        String original_doc_attribute_name = getParameterAsString(ORIGINAL_DOC_ATTRIBUTE_NAME);
        String map_doc_attribute_name = getParameterAsString(MAP_DOC_ATTRIBUTE_NAME);
        ...
        Attribute doc_attribute = attributes.get(original_doc_attribute_name);
        Attribute map_doc_attribute = attributes.get(map_doc_attribute_name);
        ...
        for (int i = 0; i < exampleSet.size(); i++) {
            ...
            String originalDoc = example.getValueAsString(doc_attribute);
            String mapDoc = example.getValueAsString(map_doc_attribute);
            double distance = TextSimilarity.computeDistance(originalDoc, mapDoc, " ",METHODS[method], ssk_subsequence_length, ssk_lambda);
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        types.add(new ParameterTypeAttribute(ORIGINAL_DOC_ATTRIBUTE_NAME, "The first attribute name of doc.", exampleSetInput,
                false));
        types.add(new ParameterTypeAttribute(MAP_DOC_ATTRIBUTE_NAME, "The second attribute name of doc.", exampleSetInput,
                false));
