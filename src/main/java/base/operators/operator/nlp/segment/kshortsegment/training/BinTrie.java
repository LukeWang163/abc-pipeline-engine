...
public class BinTrie<V> extends BaseNode<V> implements ITrie<V>, Externalizable {
	...
	private HashMap<String, HashMap> posTransMap = new HashMap();
	...
	public HashMap<String, HashMap> getPosTransMap() {
		return posTransMap;
	}

	public HashMap getTransMapByComText(String comText) {
		return posTransMap.get(comText);
	}

	// 实现词与词之间不同词性间转移概率的统计
	public void incrementTransCount(String comText, String srcPos,
			String destPos) {
		String posLabel = srcPos + "##" + destPos;// 前后词性拼合作为主键
		HashMap posMap = posTransMap.get(comText);// 词与词拼合后的字符串作为两个词间转换概率的统计主键,在上面放置Map用以存储不同的词性转移概率
		if (posMap == null)
			posMap = new HashMap();
		Integer count = (Integer) posMap.get(posLabel);
		if (count == null) {
			count = 1;
		} else {
			count++;
		}
		posMap.put(posLabel, count);
		posTransMap.put(comText, posMap);
	}

	public String getPosTransResult(String comText) {
		String txt = "";
		// String posLabel = srcPos + "##" + destPos;// 前后词性拼合作为主键
		HashMap posMap = posTransMap.get(comText);// 词与词拼合后的字符串作为两个词间转换概率的统计主键,在上面放置Map用以存储不同的词性转移概率
		...
		while (it.hasNext()) {
			String posLabel = (String) it.next();
			Integer count = (Integer) posMap.get(posLabel);
			if (count != null) {
				txt = txt+"\t" + posLabel + "##" + count;
			}
		}
		if(posMap.size()>1) multxt++;
		return txt;
	...
	public void put(String key, V value) {
		...
		// 最后一个字加入时属性为end
		if (branch.addChild(new Node<V>(chars[chars.length - 1],
				Status.WORD_END_3, value))) {
			++size; // 维护size
		}
	}
	...
	public void put(char[] key, V value) {
		...
		// 最后一个字加入时属性为end
		if (branch.addChild(new Node<V>(key[key.length - 1], Status.WORD_END_3,
				value))) {
			++size; // 维护size
		}
	}
	...
	public void remove(String key) {
		...
		// 最后一个字设为undefined
		if (branch.addChild(new Node(chars[chars.length - 1],
				Status.UNDEFINED_0, value))) {
			--size;
		}
	}
	...
	public V[] getValueArray(V[] a) {
		if (a.length < size)
			a = (V[]) java.lang.reflect.Array.newInstance(a.getClass()
	...
	public LinkedList<Map.Entry<String, V>> commonPrefixSearchWithValue(
		...
		for (int i = begin; i < chars.length; ++i) {
			...
			if (branch == null || branch.status == Status.UNDEFINED_0)
				return result;
			sb.append(aChar);
			if (branch.status == Status.WORD_MIDDLE_2
					|| branch.status == Status.WORD_END_3) {
				result.add(new AbstractMap.SimpleEntry<String, V>(
	 ...
	 *            路径
	 * @param value
	 *            额外提供的值数组，按照值的字典序。（之所以要求提供它，是因为泛型的保存不归树管理）
	 * @return 是否成功
	 */
