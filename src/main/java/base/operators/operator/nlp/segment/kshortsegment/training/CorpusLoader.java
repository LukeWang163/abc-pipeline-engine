...
import base.operators.operator.nlp.segment.kshortsegment.training.document.Sentence;
...
public class CorpusLoader {
	...
	public static void walk(ExampleSet exampleSet, String docContentColumn, Handler handler) {

		for (Example example : exampleSet) {
			//	System.out.print(file);
			Document document = Document.create(example.getValueAsString(exampleSet.getAttributes().get(docContentColumn)));
			try{
				handler.handle(document);
			}catch(RuntimeException e){
				System.out.println("Error text:"+ example.getValueAsString(exampleSet.getAttributes().get(docContentColumn)));
				e.printStackTrace();
			}
	...
	public static void walk(String folderPath, HandlerThread[] threadArray) {
		...
		for (int i = 0; i < threadArray.length - 1; ++i) {
			threadArray[i].fileList = fileList.subList(fileList.size() / threadArray.length * i,
					fileList.size() / threadArray.length * (i + 1));
			threadArray[i].start();
		}
		threadArray[threadArray.length - 1].fileList = fileList
				.subList(fileList.size() / threadArray.length * (threadArray.length - 1), fileList.size());
	...
	public static Document convert2Document(File file) {
		...
		//	System.exit(-1);
			System.out.println("=====document null:"+file.getAbsolutePath());
		}
		return null;
	}
