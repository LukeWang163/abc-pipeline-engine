...
//			}
	...
	public void readFromDir(File input) {
        ...
        for( File temp : dirFiles) {
            ...
            //查找指定的文件
            if( temp.isFile() && !temp.isHidden() ){
                //不处理超过255字节大小的文件
                if( temp.getName().length() > 255 ){
                    continue;
                }
                LineReader lineReader = null;
				try {
					lineReader = lineReaderClass_.getConstructor(String.class, String.class).newInstance(temp.getAbsolutePath(), charsetName_);
					long minThreshold = 1;
					String[] lineTokens;
        ...
        //目录处理完毕后添加类别标签
        String label = input.getAbsolutePath().substring(args_.input.length(), input.getAbsolutePath().length());
        label = label.replace(File.separator,"#");
        if(label.startsWith("#")){
        	label = label.substring(1);
        }
        add(args_.label+label);
	}
	public void readFromArgs(List<String> text) {
		try {
