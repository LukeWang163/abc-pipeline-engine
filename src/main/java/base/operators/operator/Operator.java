...
import base.operators.io.process.XMLExporter;
...
import base.operators.operator.annotation.ResourceConsumer;
...
import base.operators.operator.ports.impl.InputPortsImpl;
import base.operators.operator.ports.impl.OutputPortsImpl;
...
import base.operators.operator.preprocessing.filter.AbstractDateDataProcessing;
...
import base.operators.tools.math.StringToMatrixConverter;
import base.operators.tools.patterns.Visitor;
...
public abstract class Operator extends AbstractObservable<Operator>
		implements  LoggingHandler, ParameterHandler, ResourceConsumer {
	...
	public AtomicInteger writeCount = new AtomicInteger();
...
//	}
	...
	public Map<String, String> outPaths = new TreeMap<>();
...
//	public LogStringHandler myHandler = new LogStringHandler(new LogFormatter());

	public Logger myLog = new OperatorLogMapper(Operator.class.getName());
//    {
//        try {
...
//    }
	...
	public Operator(OperatorDescription description) {
		...
		//this.addObserver(new OperatorStatusObserver<>(), false);
...
//		}
	 ...
	 *
	 * @deprecated use
	 *             {@link base.operators.io.process.GUIProcessXMLFilter#addOperatorAnnotation(base.operators.gui.flow.processrendering.annotations.model.OperatorAnnotation)}
	 ...
	 *
	 * @deprecated use
	 *             {@link base.operators.io.process.GUIProcessXMLFilter#lookupOperatorAnnotations(Operator)}
...
//					}
	...
	public void addMyObservers(List<Observer<Operator> > list){
		list.forEach(o->addObserver(o, false));
	}
	/**
...
//		logger.setLevel(Level.ALL);
		...
		addMyObservers(process.idswObservers);
		...
		if (isEnabled()) {
			...
			try {
				...
				if (!(operatorException instanceof UserError)) {
					getLogger().severe(operatorException.getMessage());
					isError = true;
					fireUpdate();
...
//				}
	 ...
	 */
	//@Override
	public File getParameterAsFile(String key) throws UserError {
...
//	}
	...
	public String getXML(boolean hideDefault, boolean onlyCoreElements) {
		try {
			...
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
					"base.operators.operator.Operator.generating_xml_process_error", e), e);
			return e.toString();
		}
	}
	 ...
	 * {@link base.operators.gui.animation.OperatorAnimationProcessListener#processFinishedOperator}
