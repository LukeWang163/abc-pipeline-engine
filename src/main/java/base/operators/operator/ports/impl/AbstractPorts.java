...
public abstract class AbstractPorts<T extends Port> extends AbstractObservable<Port> implements Ports<T> {
	...
	public T getPortByName(String name) {
		...
		if (port != null) {
			...
			LogService.getRoot().log(Level.FINE, "base.operators.operator.ports.impl.AbstractPorts.port_does_not_exist",
					name);
				...
				for (PortExtender extender : portExtenders) {
					...
					if (name.startsWith(prefix)) {
						...
						LogService.getRoot().log(Level.FINE,
								"base.operators.operator.ports.impl.AbstractPorts.found_extender", prefix);
						try {
							...
							if (secondTry == null) {
								...
								LogService.getRoot().log(Level.WARNING,
										"base.operators.operator.ports.impl.AbstractPorts.port_extender_did_not_extend",
										new Object[] { prefix, index });
							} else {
								// LogService.getRoot().fine("Port was created. Ports are now:
								// "+getAllPorts());
								LogService.getRoot().log(Level.FINE,
										"base.operators.operator.ports.impl.AbstractPorts.ports_created", getAllPorts());
							}
							return secondTry;
							...
							LogService.getRoot().log(Level.WARNING,
									I18N.getMessage(LogService.getRoot().getResourceBundle(),
											"base.operators.operator.ports.impl.AbstractPorts.extending_error", prefix, e),
									e);
							return null;
						}
					}
	...
	public List<PortExtender> getPortExtenders(){
		return portExtenders;
	}
