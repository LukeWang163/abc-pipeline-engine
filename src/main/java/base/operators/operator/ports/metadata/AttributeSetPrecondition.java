				...
				if (!attributeName.contains("%{")) {
					...
					if (attInfo == MetaDataInfo.NO) {
						...
						if (!Ontology.ATTRIBUTE_VALUE_TYPE.isA(amd.getValueType(), requiredAttributesType)) {
							...
							if (fix != null) {
								createError(ProcessSetupError.Severity.ERROR, Collections.singletonList(fix), "attribute_has_wrong_type",
										amd.getName(), Ontology.VALUE_TYPE_NAMES[requiredAttributesType]);
							} else {
								createError(ProcessSetupError.Severity.ERROR, "attribute_has_wrong_type", amd.getName(),
