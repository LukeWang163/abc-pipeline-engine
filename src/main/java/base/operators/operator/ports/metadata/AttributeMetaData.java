...
public class AttributeMetaData implements Serializable {
	...
	private void appendValueSetDescription(StringBuilder buf) {
		if (isNominal()) {
			...
			for (String value : valueSet) {
				...
				buf.append(base.operators.tools.Tools.escapeHTML(value));
				if (isMode) {
					buf.append("</span>");
				}
			}
		...
		if (isNumerical()) {
			...
			if (getValueRange() != null) {
				buf.append(base.operators.tools.Tools.formatNumber(getValueRange().getLower(), 3));
				buf.append("...");
				buf.append(base.operators.tools.Tools.formatNumber(getValueRange().getUpper(), 3));
				buf.append("]");
			}
			if (getMean().isKnown()) {
				buf.append("; mean ");
		...
		if (valueRange != null && Ontology.ATTRIBUTE_VALUE_TYPE.isA(getValueType(), Ontology.DATE_TIME)
			...
			switch (getValueType()) {
				case Ontology.DATE:
					buf.append(base.operators.tools.Tools.formatDate(new Date((long) getValueRange().getLower())));
					buf.append("...");
					buf.append(base.operators.tools.Tools.formatDate(new Date((long) getValueRange().getUpper())));
					buf.append("]");
					break;
				case Ontology.TIME:
					buf.append(base.operators.tools.Tools.formatTime(new Date((long) getValueRange().getLower())));
					buf.append("...");
					buf.append(base.operators.tools.Tools.formatTime(new Date((long) getValueRange().getUpper())));
					buf.append("]");
					break;
				case Ontology.DATE_TIME:
					buf.append(base.operators.tools.Tools.formatDateTime(new Date((long) getValueRange().getLower())));
					buf.append("...");
					buf.append(base.operators.tools.Tools.formatDateTime(new Date((long) getValueRange().getUpper())));
					buf.append("]");
					break;
			}
		}
	...
	protected String getDescriptionAsTableRow() {
		...
		b.append(base.operators.tools.Tools.escapeHTML(getName()));
