...
import base.operators.operator.preprocessing.IdTagging;
import base.operators.parameter.UndefinedParameterError;
import base.operators.tools.Ontology;
import base.operators.tools.OperatorService;
import base.operators.operator.ModelApplier;
			...
			if (metaData instanceof ExampleSetMetaData) {
				...
				for (String attributeName : requiredAttributes) {
					...
					if (attInfo == MetaDataInfo.NO) {
						createError(ProcessSetupError.Severity.WARNING, "missing_attribute", attributeName);
					}
				}
					...
					for (String name : requiredSpecials) {
						...
						switch (has) {
							case NO:
								...
								if (fixes.size() > 0) {
									inputPort.addError(new SimpleMetaDataError(ProcessSetupError.Severity.ERROR, inputPort, fixes,
											"exampleset.missing_role", name));
								} else {
									createError(ProcessSetupError.Severity.ERROR, "special_missing", new Object[] { name });
								}
								break;
							case UNKNOWN:
								createError(ProcessSetupError.Severity.WARNING, "special_unknown", new Object[] { name });
								break;
							case YES:
								// checking type
								...
								if (!Ontology.ATTRIBUTE_VALUE_TYPE.isA(amd.getValueType(), allowedSpecialsValueType)) {
									createError(ProcessSetupError.Severity.ERROR, "special_attribute_has_wrong_type", amd.getName(), name,
