					...
					if (containsRelation == MetaDataInfo.YES) {
						...
						if (Ontology.ATTRIBUTE_VALUE_TYPE.isA(amd.getValueType(), attributeType)) {
							if (attributeRole != null && !attributeRole.equals(amd.getRole())) {
								createError(ProcessSetupError.Severity.ERROR, "attribute_must_have_role", attributeName, attributeRole);
							}
						} else {
							createError(ProcessSetupError.Severity.ERROR, "attribute_has_wrong_type", attributeName,
						...
						if (containsRelation == MetaDataInfo.UNKNOWN) {
							...
							createError(ProcessSetupError.Severity.ERROR, "missing_attribute", attributeName);
						}
					}
				}
