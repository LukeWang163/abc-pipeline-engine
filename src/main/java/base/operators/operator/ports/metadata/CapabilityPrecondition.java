...
import base.operators.operator.ports.quickfix.ChangeAttributeRoleQuickFix;
...
import base.operators.operator.preprocessing.discretization.AbstractDiscretizationOperator;
import base.operators.operator.preprocessing.filter.MissingValueReplenishment;
import base.operators.operator.preprocessing.filter.NominalToBinominal;
import base.operators.operator.preprocessing.filter.NominalToNumeric;
...
public class CapabilityPrecondition extends ExampleSetPrecondition {
	...
	public void makeAdditionalChecks(ExampleSetMetaData metaData) {
		...
		if (!capabilityProvider.supportsCapability(OperatorCapability.WEIGHTED_EXAMPLES)) {
			switch (metaData.hasSpecial(Attributes.WEIGHT_NAME)) {
				case YES:
					createError(ProcessSetupError.Severity.WARNING, "learner_does_not_support_weights");
					break;
				case NO:
				case UNKNOWN:
				default:
				...
				switch (metaData.hasSpecial(Attributes.LABEL_NAME)) {
					case UNKNOWN:
						getInputPort().addError(
								new SimpleMetaDataError(ProcessSetupError.Severity.WARNING, getInputPort(), Collections
					...
					case NO:
						getInputPort().addError(
								new SimpleMetaDataError(ProcessSetupError.Severity.ERROR, getInputPort(), Collections
										.singletonList(new ChangeAttributeRoleQuickFix(getInputPort(),
												Attributes.LABEL_NAME, "change_attribute_role", Attributes.LABEL_NAME)),
						...
						if (label.isNominal()) {
							...
							if (capabilityProvider.supportsCapability(OperatorCapability.ONE_CLASS_LABEL)
								...
								if (label.getValueSet().size() > 1 && label.getValueSetRelation() != SetRelation.UNKNOWN) {
									createError(ProcessSetupError.Severity.ERROR, "one_class_label_invalid", label.getValueSetRelation()
								...
								if (label.getValueSet().size() == 1 && label.getValueSetRelation() == SetRelation.EQUAL) {
									createError(ProcessSetupError.Severity.ERROR, "no_polynomial_label");
	...
	protected void createLearnerError(String description, List<? extends QuickFix> list) {
		...
		createError(ProcessSetupError.Severity.ERROR, list, "learner_cannot_handle", new Object[] { id, description });
	}
}
