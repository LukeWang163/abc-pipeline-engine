...
import base.operators.operator.ports.metadata.GeneratePredictionModelTransformationRule;
...
public class SDRulesetInduction extends OperatorChain {
	...
	private double[] prepareWeights(ExampleSet exampleSet) {
		Attribute weightAttr = base.operators.example.Tools.createWeightAttribute(exampleSet);
		...
		if (additive && (timesCoveredAttrib = exampleSet.getAttributes().get(TIMES_COVERED)) == null) {
			timesCoveredAttrib = base.operators.example.Tools.createSpecialAttribute(exampleSet, TIMES_COVERED,
					Ontology.INTEGER);
