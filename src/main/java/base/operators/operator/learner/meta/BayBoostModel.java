...
import base.operators.operator.AbstractModel;
...
public class BayBoostModel extends PredictionModel implements MetaModel {
			...
			if (progress != null) {
				try {
					...
					LogService.getRoot().log(Level.WARNING,
							"base.operators.operator.learner.meta.BayBoostModel.couldnt_create_operator");
				}
	...
	private Attribute[] createSpecialAttributes(ExampleSet exampleSet) {
		...
		for (int i = 0; i < specialAttributes.length; i++) {
			specialAttributes[i] = base.operators.example.Tools.createSpecialAttribute(exampleSet, attributePrefix + i,
					Ontology.NUMERICAL);
		}
	...
	public static boolean adjustIntermediateProducts(double[] products, double[] liftFactors) {
		L: for (int i = 0; i < liftFactors.length; i++) {
			...
			if (Double.isNaN(liftFactors[i])) { // WeightedPerformanceMeasures.RULE_DOES_NOT_APPLY)
				...
				LogService.getRoot().log(Level.WARNING,
						"base.operators.operator.leaner.meta.BayBoostModel.ignoring_non_applicable_model");
				continue L;
				...
				if (Double.isNaN(products[i])) {
					...
					// LogService.WARNING);
					LogService.getRoot().log(Level.WARNING,
							"base.operators.operator.leaner.meta.BayBoostModel.found_nan_value");
				}
			}
		}
		return false;
