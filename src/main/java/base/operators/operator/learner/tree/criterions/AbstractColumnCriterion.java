...
*/
package base.operators.operator.learner.tree.criterions;

import base.operators.operator.OperatorException;
import base.operators.operator.learner.tree.AbstractParallelTreeLearner;
import base.operators.operator.learner.tree.ColumnExampleTable;
import base.operators.operator.learner.tree.ColumnFrequencyCalculator;
import base.operators.parameter.ParameterHandler;
import base.operators.tools.Tools;
import base.operators.operator.learner.tree.MinimalGainHandler;
...
public abstract class AbstractColumnCriterion implements ColumnCriterion {
	...
	public static ColumnCriterion createColumnCriterion(ParameterHandler handler, double minimalGain)
		...
		String criterionName = handler.getParameterAsString(AbstractParallelTreeLearner.PARAMETER_CRITERION);
		Class<?> criterionClass = null;
		for (int i = 0; i < AbstractParallelTreeLearner.CRITERIA_NAMES.length; i++) {
			if (AbstractParallelTreeLearner.CRITERIA_NAMES[i].equals(criterionName)) {
				criterionClass = AbstractParallelTreeLearner.CRITERIA_CLASSES[i];
			}
		}
