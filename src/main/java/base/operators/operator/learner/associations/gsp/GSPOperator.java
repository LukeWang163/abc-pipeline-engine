...
*/
package base.operators.operator.learner.associations.gsp;
...
public class GSPOperator extends Operator {
				...
				if (metaData instanceof ExampleSetMetaData) {
					...
					for (AttributeMetaData amd : emd.getAllAttributes()) {
						...
						if (!Ontology.ATTRIBUTE_VALUE_TYPE.isA(amd.getValueType(), Ontology.NOMINAL)) {
							createError(ProcessSetupError.Severity.ERROR, "regular_type_mismatch",
	...
	public void doWork() throws OperatorException {
		...
		if (numberOfSequences * minSupport < 5) {
			...
			LogService.getRoot().log(Level.WARNING,
					"base.operators.operator.learner.associations.gsp.GSPOperator.number_of_sequences_founded",
					numberOfSequences);
		}
		...
		while (seeds.size() > 0) {
			...
			LogService.getRoot().log(Level.INFO,
					"base.operators.operator.learner.associations.gsp.GSPOperator.filtered_candidates", candidates.size());
	...
	private int[] countSupportingCustomer(ArrayList<Sequence> candidates, ArrayList<DataSequence> dataSequences,
		...
		LogService.getRoot().log(Level.INFO,
				"base.operators.operator.learner.associations.gsp.GSPOperator.building_hashtree_for_counting_candidates",
		...
		// LogService.INIT);
		LogService.getRoot().log(Level.INFO,
				"base.operators.operator.learner.associations.gsp.GSPOperator.counting_supporting_sequences_for_candidates",
				candidates.get(0).getNumberOfItems());
	...
	private static ArrayList<Sequence> generateCandidates(HashSet<Sequence> seeds, boolean isFirstRound) {
		...
		LogService.getRoot().log(Level.INFO,
				"base.operators.operator.learner.associations.gsp.GSPOperator.generating_candidates",
		...
		// LogService.INIT);
		LogService.getRoot().log(Level.INFO,
				"base.operators.operator.learner.associations.gsp.GSPOperator.generating_candidates", candidates.size());
		return candidates;
	}
