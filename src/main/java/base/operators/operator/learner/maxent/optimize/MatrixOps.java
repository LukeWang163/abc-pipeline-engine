 ...
 * description:A class of static utility functions for manipulating arrays of double.
 */
public final class MatrixOps
{
  ...
  public static void plusEquals (double[][] m1, double[][] m2,  double factor)
  {
    ...
    /** If "ifSelected" is false, it reverses the selection.  If
     ...
     "ifSelected" is false. */
    public static final void rowSetAll (double[] m, int nc, int ri, double v, boolean ifSelected) {
        if (ifSelected == true) {
    ...
    public static double rowDotProduct (double[] m, int nc, int ri,
                                        Example v, int maxCi) {
        return rowDotProduct (m, nc, ri, v, 1, maxCi);
    }

    public static double rowDotProduct (double[] m, int nc, int ri,
                                        Example v, double factor, int maxCi) {

        double ret = 0;
        List<Attribute> regularAttributes = Arrays.asList(v.getAttributes().createRegularAttributeArray());
        for (int i = 0; i < regularAttributes.size(); i++) {
            if (i <= maxCi){
                ret += m[ri*nc+i] * v.getValue(regularAttributes.get(i)) * factor;
            }
        }
        return ret;
    }
    ...
    public static final void rowPlusEquals (double[] m, int nc, int ri,
                                            Example v, double factor) {
        List<Attribute> regularAttributes = Arrays.asList(v.getAttributes().createRegularAttributeArray());
        for (int i = 0; i < regularAttributes.size(); i++) {
            m[ri * nc + i] += v.getValue(regularAttributes.get(i)) * factor;
        }
    }
    ...
    public static double sum(Example v) {
        double sum = 0;
        List<Attribute> regularAttributes = Arrays.asList(v.getAttributes().createRegularAttributeArray());
        for (int vli = 0; vli < regularAttributes.size(); vli++) {
            sum = sum +  v.getValue(regularAttributes.get(vli));
        }
        return sum;
    }
  ...
  public static void expNormalize(double[] scores) {
    ...
    public static double dotProduct (Example example, double[] v) {
        double ret = 0;
        Attribute[] regularAttributes = example.getAttributes().createRegularAttributeArray();
        for (int i = 0; i < regularAttributes.length; i++)
            ret += example.getValue(regularAttributes[i]) * v[i];
        return ret;
    }
}
