 ...
 */
package base.operators.operator.learner.functions.linear;
...
import base.operators.operator.learner.functions.LinearRegressionModel;
...
import base.operators.tools.math.MathFunctions;
...
public class LinearRegression extends AbstractLearner {
	...
	public Model learn(ExampleSet exampleSet) throws OperatorException {
		...
		base.operators.example.Tools.onlyNonMissingValues(exampleSet, getOperatorClassName(), this, Attributes.LABEL_NAME);
		...
		LinearRegressionMethod.LinearRegressionResult result = method.applyMethod(this, useBias, ridge, exampleSet, isUsedAttribute,
		...
		if (degreeOfFreedom > 0) {
			...
			try {
				...
				for (int i = 0; i < result.isUsedAttribute.length; i++) {
					if (result.isUsedAttribute[i]) {
						...
						if (!base.operators.tools.Tools.isZero(standardErrors[index]) && fdistribution != null) {
							...
							if (base.operators.tools.Tools.isZero(result.coefficients[index])) {
			...
			if (!base.operators.tools.Tools.isZero(standardErrors[standardErrors.length - 1]) && fdistribution != null) {
	...
	double[] performRegression(ExampleSet exampleSet, Attribute[] usedAttributes, Attribute label, double[] means,
		...
		if (usedAttributes.length > 0) {
			double[] coefficientsWithoutIntercept = base.operators.tools.math.LinearRegression.performRegression(independent,
