...
import static base.operators.parameter.ParameterTypeDateFormat.PARAMETER_DATE_FORMAT;
...
import base.operators.operator.OperatorRuntimeException;
import base.operators.operator.UserError;
import base.operators.operator.generator.ExampleSetGenerator;
...
public class DataResultSetTranslationConfiguration {
	...
	public void reconfigure(AbstractDataResultSetReader readerOperator) {
		...
		if (readerOperator != null) {
			...
			boolean firstRowAsNames = readerOperator.getParameterAsBoolean(AbstractDataResultSetReader.PARAMETER_FIRST_ROW_AS_NAMES);
			if (firstRowAsNames) {
				annotationsMap.put(0, AbstractDataResultSetReader.ANNOTATION_NAME);
			} else {
				List<String[]> annotations;
				try {
					annotations = readerOperator.getParameterList(AbstractDataResultSetReader.PARAMETER_ANNOTATIONS);
			...
			try {
				int localeIndex;
				localeIndex = readerOperator.getParameterAsInt(AbstractDataResultSetReader.PARAMETER_LOCALE);
	...
	public void setParameters(AbstractDataResultSetReader operator) {
		...
		operator.getParameters().setParameter(AbstractDataResultSetReader.PARAMETER_META_DATA, ParameterTypeList.transformList2String(metaDataList));
		...
		operator.setParameter(AbstractDataResultSetReader.PARAMETER_ANNOTATIONS, ParameterTypeList.transformList2String(annotationList));

		operator.getParameters().setParameter(AbstractDataResultSetReader.PARAMETER_ERROR_TOLERANT,
				String.valueOf(isFaultTolerant()));
		operator.getParameters().setParameter(AbstractDataResultSetReader.PARAMETER_FIRST_ROW_AS_NAMES, "false");
	}
	...
	public static ColumnMetaData[] readColumnMetaData(AbstractDataResultSetReader readerOperator) {
		...
		if (readerOperator.isParameterSet(AbstractDataResultSetReader.PARAMETER_META_DATA) && !readerOperator.getParameterAsBoolean(AbstractDataResultSetReader.PARAMETER_READ_AS_POLYNOMINAL)) {
			try {
				metaDataSettings = readerOperator.getParameterList(AbstractDataResultSetReader.PARAMETER_META_DATA);
			} catch (UndefinedParameterError e) {
