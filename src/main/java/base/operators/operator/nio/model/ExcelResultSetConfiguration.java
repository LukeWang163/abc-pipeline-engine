 ...
 */
package base.operators.operator.nio.model;

import base.operators.operator.Operator;
import base.operators.operator.OperatorException;
import base.operators.operator.UserError;
import base.operators.operator.nio.ExcelExampleSource;
import base.operators.operator.nio.ExcelSheetTableModel;
import base.operators.operator.nio.model.xlsx.XlsxResultSet;
import base.operators.operator.nio.model.xlsx.XlsxSheetTableModel;
import base.operators.operator.nio.model.xlsx.XlsxWorkbookParser;
...
public class ExcelResultSetConfiguration implements DataResultSetFactory, ExcelSheetSelection {
	...
	public ExcelResultSetConfiguration(ExcelExampleSource excelExampleSource) throws OperatorException {
		...
		this.fileName = excelExampleSource.getParameter(ExcelExampleSource.PARAMETER_EXCEL_FILE);
...
//				}
				if("local".equals(excelExampleSource.getParameter(ExcelExampleSource.PARAMETER_STORAGE_TYPE).toLowerCase())){
					try {
						this.fileInputStream = new FileInputStream(new File(excelParamter));
					} catch (FileNotFoundException e) {
						e.printStackTrace();
					}
				}else if("hdfs".equals(excelExampleSource.getParameter(ExcelExampleSource.PARAMETER_STORAGE_TYPE).toLowerCase())){
					FileSystem fs = HDFSUtil.getFileSystem();
					try {
						this.fileInputStream = fs.open(new Path(excelParamter));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
	 ...
	 */
	public AbstractTableModel createExcelTableModel(int sheetIndex, XlsxResultSet.XlsxReadMode readMode, ProgressListener progressListener)
	 ...
	 * @throws InvalidFormatException
	 */
	public AbstractTableModel createExcelTableModel(ExcelSheetSelection sheetSelection, XlsxResultSet.XlsxReadMode readMode, ProgressListener progressListener)
			throws BiffException, IOException, InvalidFormatException, OperatorException, ParseException {
	...
	private int getNumberOfSheets(XlsxWorkbookParser.XlsxWorkbook workbook) {
		return workbook.xlsxWorkbookSheets.size();
	}

	private XlsxWorkbookParser.XlsxWorkbook parseWorkbook(ZipFile zipFile)
	...
	public String[] getSheetNames() throws BiffException, IOException, InvalidFormatException, UserError {
		if (getFile().getAbsolutePath().endsWith(XLSX_FILE_ENDING)) {
			// excel 2007 file
			try (ZipFile zipFile = new ZipFile(getFile().getAbsolutePath())) {
				XlsxWorkbookParser.XlsxWorkbook workbook;
				try {
	...
	public DataResultSet makeDataResultSet(Operator operator) throws OperatorException {
		return makeDataResultSet(operator, XlsxResultSet.XlsxReadMode.OPERATOR);
	}

	/**
	 * Creates a {@link DataResultSet} based on the current configuration and the provided
	 * {@link XlsxResultSet.XlsxReadMode}.
	 *
	 * @param operator
	 ...
	 */
	public DataResultSet makeDataResultSet(Operator operator, XlsxResultSet.XlsxReadMode readMode, DateFormatProvider provider)
			throws OperatorException {
		File file = getFile();
		...
		if (file != null) {
			...
			if (this.fileName.endsWith(XLSX_FILE_ENDING)) {
				resultSet = createExcel2007ResultSet(operator, readMode, provider);
			} else if (this.fileName.endsWith(XLS_FILE_ENDING)) {
				// excel pre 2007 file
				resultSet = new ExcelResultSet(operator, this, provider);
			} else {
				// we might also get a file object that has neither .xlsx nor .xls as file ending,
				...
				// see if it works. If it does not work, it's an excel 2007 file.
				try {
					Workbook.getWorkbook(this.fileInputStream);
					resultSet = new ExcelResultSet(operator, this, provider);
				} catch (Exception e) {
					resultSet = createExcel2007ResultSet(operator, readMode, provider);
				}
	 ...
	 *             in case the creation fails because of an invalid configuration
	 */
	public DataResultSet makeDataResultSet(Operator operator, XlsxResultSet.XlsxReadMode readMode) throws OperatorException {
	...
	private DataResultSet createExcel2007ResultSet(Operator operator, XlsxResultSet.XlsxReadMode readMode, DateFormatProvider provider)
			throws OperatorException {
	...
	private XlsxResultSet createXLSXResultSet(Operator operator, XlsxResultSet.XlsxReadMode readMode, DateFormatProvider provider)
			throws UserError {
	...
	public TableModel makePreviewTableModel(ProgressListener listener) throws OperatorException {
		...
		if (absolutePath.endsWith(XLSX_FILE_ENDING)) {
			try {
				return createExcelTableModel(getSheetSelectionMethod(), XlsxResultSet.XlsxReadMode.WIZARD_PREVIEW, listener);
