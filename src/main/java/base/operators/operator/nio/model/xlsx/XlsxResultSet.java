...
*/
package base.operators.operator.nio.model.xlsx;
...
import base.operators.operator.nio.model.ExcelSheetSelection;
...
public class XlsxResultSet implements DataResultSet {
	...
	private final XlsxWorkbookParser.XlsxWorkbook xlsxWorkbook;
	...
	public XlsxResultSet(Operator callingOperator, final ExcelResultSetConfiguration configuration, ExcelSheetSelection sheetSelection,
		...
		try {
			...
			XlsxWorkbookRelationParser.XlsxWorkbookRel workbookRelations;
	...
	private String getValue(int columnIndex) {
		XlsxUtilities.XlsxCell xlsxCell = getXlsxCell(columnIndex);
		return xlsxCell == null ? null : xlsxCell.value;
	}

	/**
	 * @return the {@link XlsxUtilities.XlsxCell} for the specified column
	 */
	private XlsxUtilities.XlsxCell getXlsxCell(int columnIndex) {
		XlsxUtilities.XlsxCell[] rowContent = worksheetParser.getRowContent();
		if (rowContent != null) {
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING,
					I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.operator.nio.model.ExcelResultSetConfiguration.close_workbook_error",
							e.getMessage()),
					e);
		}
	}
