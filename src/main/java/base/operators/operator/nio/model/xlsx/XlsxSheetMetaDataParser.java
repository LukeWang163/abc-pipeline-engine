...
public class XlsxSheetMetaDataParser {
	...
	public XlsxSheetMetaData parseMetaData(Operator callingOperator, ExcelResultSetConfiguration configuration,
                                           XlsxResultSet.XlsxReadMode readMode) throws XMLStreamException, IOException, UserError {
		...
		if (readMode != XlsxResultSet.XlsxReadMode.WIZARD_WORKPANE) {
					...
					switch (reader.getLocalName()) {
						case TAG_DIMENSION:
							...
							if (dimColonIndex != -1) {
								...
								try {
									XlsxUtilities.XlsxCellCoordinates cellRange = XlsxUtilities.convertCellRefToCoordinates(maxCellRange);
						...
						case XlsxUtilities.TAG_CELL:
							...
							if (isRowWithoutSpan) {
								XlsxUtilities.XlsxCellCoordinates columnAndRowIndices = XlsxUtilities
