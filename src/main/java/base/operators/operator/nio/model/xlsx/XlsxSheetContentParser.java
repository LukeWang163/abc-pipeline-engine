...
class XlsxSheetContentParser implements AutoCloseable {
	...
	private XlsxUtilities.XlsxCell[] currentRowContent;
	...
	private XlsxUtilities.XlsxCell[] nextRowWithContent;
	...
	private void assignNextCurrentRow(boolean assignEmptyRow) throws ParseException, XMLStreamException {
		if (assignEmptyRow) {
			...
			this.currentRowContent = new XlsxUtilities.XlsxCell[sheetMetaData.getNumberOfColumns()];
		} else {
	 ...
	 *             If there is an error on parsing a single XML item.
	 */
	private XlsxUtilities.XlsxCell[] parseNextRowWithContent() throws ParseException, XMLStreamException {
		...
		XlsxUtilities.XlsxCell[] nextRowWithContent = new XlsxUtilities.XlsxCell[numberOfColumns];
					...
					if (startLocalName.equals(XlsxUtilities.TAG_ROW)) {
						...
						try {
							...
							throw new ParseException(
									new ParsingError(parsedRowIndex, columnIndex, ParsingError.ErrorCode.FILE_SYNTAX_ERROR, indexValue));
						}
						...
						if (type != null) {
							nextRowWithContent[columnIndex] = new XlsxUtilities.XlsxCell(type);
						} else {
							throw new ParseException(new ParsingError(parsedRowIndex, columnIndex,
	 ...
	 *         been parsed yet.
	 */
	XlsxUtilities.XlsxCell[] getRowContent() {
