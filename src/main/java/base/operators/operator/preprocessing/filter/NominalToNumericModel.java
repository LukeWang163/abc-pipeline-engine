...
public class NominalToNumericModel extends PreprocessingModel {
		...
		switch (unexpectedValueHandling) {
			case NominalToNumeric.ALL_ZEROES_AND_WARNING:
				LogService.getRoot().log(Level.WARNING,
						"base.operators.operator.preprocessing.filter.NominalToNumericModel.unexpected_value", targetName);
				return 0;
	...
	public String toResultString() {
		...
		builder.append(getName() + base.operators.tools.Tools.getLineSeparators(2));
		...
		builder.append("Coding Type: " + codingTypeString + base.operators.tools.Tools.getLineSeparator());
		if (!useComparisonGroups) {
			builder.append("Model covering " + trainAttributes.size() + " attributes:" + base.operators.tools.Tools.getLineSeparator());
			for (Attribute attribute : trainAttributes) {
				builder.append(" - " + attribute.getName() + base.operators.tools.Tools.getLineSeparator());
			}
		} else {
