	...
	public void addCriterion(PerformanceCriterion crit) {
		...
		if (pc != null) {
			...
			LogService
					.getRoot()
					.log(Level.WARNING,
							"base.operators.operator.performance.PerformanceVector.performance_criterion_already_part_of_performance_vector",
							crit.getName());
		}
		addAveragable(crit);
	}
	...
	public void setMainCriterionName(String mcName) {
		if ((!mcName.equals(MAIN_CRITERION_FIRST)) && (getAveragable(mcName) == null)) {
			...
			LogService.getRoot().log(Level.SEVERE,
					"base.operators.operator.performance.PerformanceVector.main_criterion_not_found", mcName);
		}
