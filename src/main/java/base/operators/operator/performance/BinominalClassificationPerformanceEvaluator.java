...
public class BinominalClassificationPerformanceEvaluator extends AbstractPerformanceEvaluator {
	...
	private static final Class<?>[] SIMPLE_CRITERIA_CLASSES = {
		AreaUnderCurve.Optimistic.class,
		AreaUnderCurve.Optimistic.class,
		AreaUnderCurve.Neutral.class,
		AreaUnderCurve.Pessimistic.class };
	...
	public List<PerformanceCriterion> getCriteria() {
		...
		for (int i = 0; i < SIMPLE_CRITERIA_CLASSES.length; i++) {
			try {
				...
				.getRoot()
				.log(Level.SEVERE,
						"base.operators.operator.performance.BinominalClassificationPerformanceEvaluator.instantiating_simple_criteria_classes_error",
