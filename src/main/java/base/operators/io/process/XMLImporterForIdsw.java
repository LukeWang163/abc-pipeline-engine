...
import base.operators.gui.tools.VersionNumber.VersionNumberException;
import base.operators.io.process.rules.*;
 ...
 *
 */
public class XMLImporterForIdsw {
	...
	public static void init() {
		URL rulesResource = XMLImporterForIdsw.class.getResource("/base/operators/resources/parserules.xml");
		if (rulesResource != null) {
			// registering the core rules without name prefix
			importParseRules(rulesResource, null);
		} else {
			LogService.getRoot().log(Level.WARNING, "base.operators.io.process.XMLImporter.cannot_find_default_parse_rules");
		}
	}
	...
	public static void importParseRules(URL rulesResource, Plugin prover) {
		if (rulesResource == null) {
			...
			LogService.getRoot().log(Level.CONFIG, "base.operators.io.process.XMLImporter.reading_parse_rules",
					rulesResource);
			try {
				...
				if (!doc.getDocumentElement().getTagName().equals("parserules")) {
					LogService.getRoot().log(Level.SEVERE, "base.operators.io.process.XMLImporter.xml_document_start_error",
							rulesResource);
				} else {
					...
					LogService.getRoot().log(Level.FINE, "base.operators.io.process.XMLImporter.found_rules",
							PARSE_RULES.size());
				}
			} catch (Exception e) {
				LogService.getRoot()
						.log(Level.SEVERE,
								I18N.getMessage(LogService.getRoot().getResourceBundle(),
										"base.operators.io.process.XMLImporter.error_reading_parse_rules", rulesResource, e),
						e);
			}
		}
	}
	...
	public XMLImporterForIdsw(ProgressListener listener) {
	 ...
	 */
	@Deprecated
	public XMLImporterForIdsw(ProgressListener listener, int version) {
		this(listener);
	}
...
//	 */
			...
			if (rootOp instanceof OperatorChain) {
				try {
					...
					LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.io.process.XMLImporter.autowire_error", e), e);
				}
			}
		}
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (child instanceof Element) {
				...
				if ("operator".equals(opElement.getTagName())) {
					parseOperator2(opElement, executionUnit, processFileVersion, process, unknownParameterInformation);
	...
	public Operator parseOperator(Element opElement, VersionNumber processVersion, Process process,
		...
		Operator operator = parseOperator2(opElement, null, processVersion, process, unknownParameterInformation);
		unlockPorts(operator);
		return operator;
	}

	private Operator parseOperator2(Element opElement, ExecutionUnit addToProcess, VersionNumber processFileVersion,
								   Process process, List<UnknownParameterInformation> unknownParameterInformation) throws XMLException {
		...
		try {
			opDescr = PluginUtil.getDescription(opElement, className);
		...
		if(opElement.hasAttribute("id")) {
			operator.setId(opElement.getAttribute("id"));
		}
				...
				if (inner.getTagName().toLowerCase().equals("parameter")) {
					...
					if (!knownType) {
						if (relevantParameter(className, parameter[0])) {
							LogService.getRoot().log(Level.INFO,
									"base.operators.io.process.XMLImporter.attribute_not_found_unknown", new Object[] {
											parameter[0], operator.getName(), operator.getOperatorDescription().getName() });
						}
					}
				} else if(inner.getTagName().toLowerCase().equals("path")) {
					final String port = inner.getAttribute("port");
					final String path = inner.getAttribute("path");
					operator.outPaths.put(port, path);
					...
					if (type == null) {
						if (relevantParameter(className, key)) {
							LogService.getRoot().log(Level.INFO,
									"base.operators.io.process.XMLImporter.attribute_not_found_list",
						...
						if (!(type instanceof ParameterTypeList)) {
							...
							if (!knownType) {
								if (relevantParameter(className, listDescription.getKey())) {
									LogService.getRoot().log(Level.INFO,
											"base.operators.io.process.XMLImporter.attribute_not_found_unknown",
					...
					if (type == null) {
						if (relevantParameter(className, key)) {
							LogService.getRoot().log(Level.INFO,
									"base.operators.io.process.XMLImporter.attribute_not_found_enum",
									new Object[] { key, operator.getName(), operator.getOperatorDescription().getName() });
						}
					} else {
						...
						if (subprocessIndex >= nop.getNumberOfSubprocesses() && !nop.areSubprocessesExtendable()) {
							...
							if ("process".equals(childProcessElement.getTagName())) {
								...
								if (processFileVersion.compareTo(VERSION_RM_5) >= 0) {
									...
									parseOperator2(childProcessElement, subprocess, processFileVersion, process,
											unknownParameterInformation);
