...
*/
package base.operators.gui.tools;
		...
		try {
			...
			LogService.getRoot().log(
					...
					I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.gui.tools.ProgressThread.executing_error", name), e);
...
//					}
		...
		return new Runnable() {
			...
			public void run() {
				synchronized (LOCK) {
					if (cancelled) {
						LogService.getRoot().log(Level.INFO, "base.operators.gui.tools.ProgressThread.task_cancelled",
								getName());
						return;
					}
					started = true;
...
//								}
				...
				try {
					...
					ActionStatisticsCollector.getInstance().log(ActionStatisticsCollector.TYPE_PROGRESS_THREAD, key,
							"stopped");
					LogService.getRoot().log(Level.FINE, "base.operators.gui.tools.ProgressThread.progress_thread_aborted",
							getName());
				} catch (Exception e) {
					showProgressTimer.cancel();
					ActionStatisticsCollector.getInstance().log(ActionStatisticsCollector.TYPE_PROGRESS_THREAD, key,
					...
					LogService.getRoot().log(
							Level.WARNING,
							I18N.getMessage(LogService.getRoot().getResourceBundle(),
									"base.operators.gui.tools.ProgressThread.error_executing_background_job", name, e), e);
