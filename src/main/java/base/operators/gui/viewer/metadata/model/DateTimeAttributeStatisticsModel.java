...
public class DateTimeAttributeStatisticsModel extends AbstractAttributeStatisticsModel {
    ...
    /** the mean of the date_time values */
    private String mean;
    ...
    public void updateStatistics(final ExampleSet exampleSet) {
        ...
        long meanMilliseconds = (long) exampleSet.getStatistics(getAttribute(), Statistics.AVERAGE);
        long difference = maxMilliseconds - minMilliseconds;
        String dura = "";
        if (getAttribute().getValueType() == Ontology.DATE) {
            // days
            dura += base.operators.tools.Tools.formatIntegerIfPossible(
                    Math.floor(difference / (H_IN_D * M_IN_H * S_IN_M * MS_IN_S)), 3)
                    + days;
        } else if (getAttribute().getValueType() == Ontology.TIME) {
            // hours
            dura += base.operators.tools.Tools.formatIntegerIfPossible(Math.floor(difference / (M_IN_H * S_IN_M * MS_IN_S)),
                    3) + hours;
            ...
            dura += base.operators.tools.Tools.formatIntegerIfPossible(Math.floor(leftoverMilliSeconds / MS_IN_S), 3)
                    + SHORT_SECOND;
        }
        String minResult = null;
        String maxResult = null;
        String meanResult = null;
...
//        fireStatisticsChangedEvent();
    ...
    /**
     * Gets the mean value (avg) of the date_time values.
     *
     * @return
     */
