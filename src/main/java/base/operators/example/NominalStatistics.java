...
public class NominalStatistics implements Statistics {
	...
	public double getStatistics(Attribute attribute, String name, String parameter) {
		if (MODE.equals(name)) {
			...
			if (parameter != null) {
				...
				LogService.getRoot().log(Level.WARNING,
						"base.operators.example.NominalStatistics.calculating_statistics_count_for_attribute_error",
						attribute.getName());
				return Double.NaN;
			}
		}
		if (LEAST.equals(name)) {
			...
			// LogService.WARNING);
			LogService.getRoot().log(Level.WARNING,
					"base.operators.example.NominalStatistics.calculating_statistics_unknown_type_error", name);
			return Double.NaN;
		}
	}
