...
public class Partition implements Cloneable, Serializable {
	...
	private void init(double[] ratio, int size, PartitionBuilder builder) {
		...
		LogService.getRoot().log(Level.FINE, "base.operators.example.set.Partition.creating_new_partition_using",
	...
	private void init(int[] newElements, int noOfPartitions) {
		...
		// LogService.STATUS);
		LogService.getRoot().log(Level.FINE, "base.operators.example.set.Partition.creating_new_partition_with",
