		...
		for (int i = 1; i < ratio.length; i++) {
			if (ratio[i] != firstValue) {
				LogService.getRoot().log(Level.FINE,
						"base.operators.example.set.StratifiedPartitionBuilder.not_all_ratio_values_are_equal");
				return createNonEqualPartition(ratio, size, label);
			}
		}
		LogService.getRoot().log(Level.FINE,
				"base.operators.example.set.StratifiedPartitionBuilder.all_ratio_values_are_equal");
