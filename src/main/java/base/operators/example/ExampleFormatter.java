	...
	public static class SimpleCommand implements FormatCommand {
		...
		public String format(Example example) {
			...
			switch (command) {
				...
				case 'd': {
					if (arguments.length != 0) {
						return base.operators.tools.Tools.formatNumber(example.getConfidence(arguments[0]), fractionDigits);
					}
					StringBuffer str = new StringBuffer();
					boolean first = true;
					for (String value : attributes.getPredictedLabel().getMapping().getValues()) {
						...
						str.append(
								"conf(" + value + ")=" + base.operators.tools.Tools.formatNumber(example.getConfidence(value), fractionDigits));
					}
					return str.toString();
				}
				default:
