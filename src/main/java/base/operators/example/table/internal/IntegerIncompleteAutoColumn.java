...
final class IntegerIncompleteAutoColumn implements Column {
	...
	static abstract class IntegerIncompleteChunk implements Serializable {
		 ...
		 */
		final ExampleSetBuilder.DataManagement management;

		IntegerIncompleteChunk(int id, IntegerIncompleteChunk[] chunks, ExampleSetBuilder.DataManagement management) {
			this.id = id;
			this.chunks = chunks;
	...
	private final ExampleSetBuilder.DataManagement management;
	 ...
	 */
	IntegerIncompleteAutoColumn(int size, ExampleSetBuilder.DataManagement management) {
		this.management = management;
		ensure(size);
	}
	...
	public void ensure(int size) {
		...
		while (rowsLeft > 0) {
			...
			if (enlargeLastChunk) {
				...
				if (management == ExampleSetBuilder.DataManagement.MEMORY_OPTIMIZED) {
