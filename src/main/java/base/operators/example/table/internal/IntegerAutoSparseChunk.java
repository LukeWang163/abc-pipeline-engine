...
/**
 * Sparse {@link IntegerAutoColumn.IntegerAutoChunk} for integer value data inside a {@link IntegerAutoColumn}.
 *
 * @author Gisa Schaefer
 * @since 7.3.1
 */
final class IntegerAutoSparseChunk extends IntegerAutoColumn.IntegerAutoChunk {
	...
	IntegerAutoSparseChunk(int id, IntegerAutoColumn.IntegerAutoChunk[] chunks, double defaultValue, ExampleSetBuilder.DataManagement management) {
		super(id, chunks, management);
		if (management == ExampleSetBuilder.DataManagement.AUTO) {
	...
	private void changeToDense(int max) {
		IntegerAutoColumn.IntegerAutoChunk dense = new IntegerAutoDenseChunk(id, chunks, ensuredSize, management);
