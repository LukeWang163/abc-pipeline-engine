...
import base.operators.example.table.internal.AutoColumnUtils.DensityResult;


/**
 * Dense {@link IntegerAutoColumn.IntegerAutoChunk} for integer value data inside a {@link IntegerAutoColumn}.
 *
 * @author Gisa Schaefer
 * @since 7.3.1
 */
final class IntegerAutoDenseChunk extends IntegerAutoColumn.IntegerAutoChunk {
	...
	IntegerAutoDenseChunk(int id, IntegerAutoColumn.IntegerAutoChunk[] chunks, int size, ExampleSetBuilder.DataManagement management) {
	...
	private void checkSparse() {
		...
		if (result.density < thresholdDensity) {
			...
			IntegerAutoColumn.IntegerAutoChunk sparse = new IntegerAutoSparseChunk(id, chunks, defaultValue, management);
