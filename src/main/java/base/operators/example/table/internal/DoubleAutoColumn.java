...
final class DoubleAutoColumn implements Column {
	...
	static abstract class DoubleAutoChunk implements Serializable {
		...
		DoubleAutoChunk(int id, DoubleAutoChunk[] chunks, ExampleSetBuilder.DataManagement management) {
			this.id = id;
			this.chunks = chunks;
	 ...
	 */
	DoubleAutoColumn(int size, ExampleSetBuilder.DataManagement management) {
		this.management = management;
		ensure(size);
	}
