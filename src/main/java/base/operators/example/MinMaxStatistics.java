...
*/
package base.operators.example;
...
public class MinMaxStatistics implements Statistics {
	...
	private double lowerQuartile = 0.0d;

	private double higherQuartile = 0.0d;
	...
	private MinMaxStatistics(MinMaxStatistics other) {
		...
		this.lowerQuartile = other.lowerQuartile;
		this.higherQuartile = other.higherQuartile;
		this.statistics = new DescriptiveStatistics();
		this.statistics = new DescriptiveStatistics(other.statistics);
	}
	...
	public double getStatistics(Attribute attribute, String name, String parameter) {
		if (MINIMUM.equals(name)) {
			...
			return this.maximum;
		} else if (HIGHER_QUARTILE.equals(name)) {
			return this.statistics.getPercentile(75);
		} else if (LOWER_QUARTILE.equals(name)) {
			return this.statistics.getPercentile(25);
		} else {
			// LogService.getRoot().warning("Cannot calculate statistics, unknown type: " + name);
			LogService.getRoot().log(Level.WARNING, "base.operators.example.MinMaxStatistics.calculating_statistics_error",
					name);
			return Double.NaN;
		}
	}
	...
	public boolean handleStatistics(String name) {
		return MINIMUM.equals(name) || MAXIMUM.equals(name) || HIGHER_QUARTILE.equals(name) || LOWER_QUARTILE.equals(name);
	}
	...
	public void startCounting(Attribute attribute) {
		...
		this.higherQuartile = 0.0d;
		this.lowerQuartile = 0.0d;
	}
}
