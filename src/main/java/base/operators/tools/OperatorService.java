...
import base.operators.operator.tools.OperatorCreationHook;
import base.operators.search.GlobalSearchable;
import base.operators.tools.documentation.OperatorDocBundle;
import base.operators.tools.documentation.OperatorDocumentation;
import base.operators.tools.documentation.XMLOperatorDocBundle;
...
public class OperatorService {
	...
	public static final String OPERATOR_BLACKLIST_KEY = "operators.operator.blacklist";
	...
	public static void init() {
		...
		operatorSearchable = null;

		URL mainOperators = getMainOperators();
		if (mainOperators == null) {
			LogService.getRoot().log(Level.SEVERE,
					"base.operators.tools.OperatorService.main_operator_descripton_file_not_found",
		...
		if (additionalOperators != null && !additionalOperators.isEmpty()) {
			if (!RapidMiner.getExecutionMode().canAccessFilesystem()) {
				LogService.getRoot().log(Level.CONFIG,
						"base.operators.tools.OperatorService.execution_mode_does_not_permitting_accessing_file_system",
						new Object[] { RapidMiner.getExecutionMode(), additionalOperators });
			} else {
				LogService.getRoot().log(Level.INFO, "base.operators.tools.OperatorService.loading_additional_operators",
						...
						try (FileInputStream in = new FileInputStream(additionalOperatorFile)) {
							...
							LogService.getRoot().log(Level.SEVERE,
									I18N.getMessage(LogService.getRoot().getResourceBundle(),
											"base.operators.tools.OperatorService.reading_additional_operator_file_error",
											additionalOperatorFile),
									e);
						}
					} else {
						LogService.getRoot().log(Level.SEVERE,
								"base.operators.tools.OperatorService.operator_description_file_not_found",
		...
		LogService.getRoot().log(Level.FINE,
				"base.operators.tools.OperatorService.number_of_registered_operator_classes_and_descriptions",
...
//			}
	...
	public static void registerOperators(URL operatorsXML, ClassLoader classLoader, Plugin plugin) {
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING,
					I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.OperatorService.opening_stream_to_operator_description_file_error",
							operatorsXML, e),
					e);
			return;
		}
	...
	public static void registerOperators(String name, InputStream operatorsXML, ClassLoader classLoader, Plugin provider) {
		...
		LogService.getRoot().log(Level.CONFIG, "base.operators.tools.OperatorService.loading_operators", name);
		String version = null;
		...
		try {
			...
			if (!document.getDocumentElement().getTagName().toLowerCase().equals("operators")) {
				LogService.getRoot().log(Level.SEVERE,
						"base.operators.tools.OperatorService.operator_description_file_outermost_tag", name);
				return;
			}
			...
			if (version.startsWith("5.") || version.startsWith("6.") || version.startsWith("7.")) {
				...
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.tools.OperatorService.operator_description_file_wrong_version", name, version));
			}
		} catch (Exception e) {
			LogService.getRoot().log(Level.SEVERE,
					I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.OperatorService.operator_description_file_reading_error", name,
							e.getMessage()),
					e);
			return;
		} finally {
			try {
				...
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.tools.OperatorService.error_closing_stream", e.getMessage()), e);
			}
		}
	}
	...
	private static void parseOperators(Document document, ClassLoader classLoader, Plugin provider)
		...
		if (docBundle == null || docBundle.isEmpty()) {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.OperatorService.operators_no_attached_documention",
					providerName);
		} else {
			...
			if (child instanceof Element) {
				...
				if (childElement.getTagName().equals("group")) {
					...
					try {
						...
						LogService.getRoot().log(Level.WARNING,
								I18N.getMessage(LogService.getRoot().getResourceBundle(),
										"base.operators.tools.OperatorService.operator_description_creating_error",
						...
						LogService.getRoot().log(Level.WARNING,
								I18N.getMessage(LogService.getRoot().getResourceBundle(),
										"base.operators.tools.OperatorService.operator_registering_error", extensionId,
					...
					if (factoryClassName == null || factoryClassName.isEmpty()) {
						LogService.getRoot().log(Level.WARNING,
								"base.operators.tools.OperatorService.malformed_operator_descriptor_factory");
					} else {
						...
						try {
							...
							LogService.getRoot().log(Level.WARNING,
									I18N.getMessage(LogService.getRoot().getResourceBundle(),
											"base.operators.tools.OperatorService.operator_factory_class_not_found",
											factoryClassName),
									e);
						}
						if (factoryClass != null) {
							if (GenericOperatorFactory.class.isAssignableFrom(factoryClass)) {
								...
								try {
									...
									LogService.getRoot().log(Level.WARNING,
											"base.operators.tools.OperatorService.operator_instantiating_error",
									...
									LogService.getRoot().log(Level.SEVERE,
											I18N.getMessage(LogService.getRoot().getResourceBundle(),
													"base.operators.tools.OperatorService.failed_to_register_oprator", e),
											e);
								}
								LogService.getRoot().log(Level.CONFIG,
										"base.operators.tools.OperatorService.creating_operators_from_factory",
										factoryClassName);
								try {
									if (factory != null) {
									...
									LogService.getRoot().log(Level.WARNING,
											I18N.getMessage(LogService.getRoot().getResourceBundle(),
													"base.operators.tools.OperatorService.error_registering_oprators_from",
								...
								LogService.getRoot().log(Level.WARNING,
										"base.operators.tools.OperatorService.malformed_operator_descriptor_subclasses",
										factoryClassName);
							}
						}
					}
	...
	public static void registerOperator(OperatorDescription description, OperatorDocBundle bundle)
		...
		if (oldDescription != null) {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.OperatorService.operator_key_already_registered",
	...
	public static Map<String, OperatorDescription> getOperatorMap() {
		return KEYS_TO_DESCRIPTIONS;
	}
...
//	}
	...
	@SuppressWarnings("unchecked")
	public static <T extends Operator> T createOperator(Class<T> clazz) throws OperatorCreationException {
		OperatorDescription description = new OperatorDescription(null, null, clazz, null, null, null);

		Constructor constructor = null;
		try {
			constructor = clazz.getConstructor(OperatorDescription.class);
			return (T) constructor.newInstance(description);
	...
	private static URL getMainOperators() {
		...
		if (operatorsXML != null) {
			...
			// + operatorsXML + ".");
			LogService.getRoot().log(Level.CONFIG,
					"OperatorService.main_operator_descriptor_overrideen", operatorsXML);
		} else {
	...
	public static void invokeCreationHooks(Operator operator) {
		for (OperatorCreationHook hook : operatorCreationHooks) {
			try {
				...
				// e);
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.tools.OperatorService.error_in_operation_creation_hook", e), e);
			}
		}
	}
