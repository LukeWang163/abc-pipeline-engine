...
*/
package base.operators.tools.plugin;
...
public class Plugin {
	 ...
	 */
	public static final String RAPIDMINER_TYPE = "Base-Type";

	/**
	 * The value for the manifest entry RapidMiner-Type which indicates that a jar file is a
	 * RapidMiner plugin.
	 */
	public static final String RAPIDMINER_TYPE_PLUGIN = "Base_Extension";

	/** @since 9.0.0 */
	public static final String PROPERTY_PLUGINS_WHITELIST = "operators.extensions.whitelist";
	/** @since 9.0.0 */
	...
	private static final String PACKAGEID_RAPIDMINER = "operators-studio-6";
	static {
	...
	private boolean checkDependencies(Plugin plugin, Collection<Plugin> plugins) {
		if (RapidMiner.getVersion().compareTo(getNecessaryRapidMinerVersion()) < 0) {
			LogService.getRoot().log(Level.WARNING,
					"base.operators.tools.plugin.Plugin.registring_operators_error_rm_version",
		...
		while (i.hasNext()) {
			...
			if (!dependency.isFulfilled(plugins)) {
				LogService.getRoot().log(Level.WARNING,
						"base.operators.tools.plugin.Plugin.registring_operators_error_ext_missing",
	...
	private void fetchMetaData() throws IOException {
		try {
			...
			requiredRapidMinerVersion = getValue(atts, "Base-Version");
	...
	public void registerOperators() {
		if (disabled) {
			LogService.getRoot().log(Level.WARNING, "Plugin.registring_operators_error",
					getName());
		}
		InputStream in = null;
		...
		if (pluginResourceOperators != null) {
			...
			if (operatorsURL == null) {
				LogService.getRoot().log(Level.WARNING,
						"base.operators.tools.plugin.Plugin.operators_description_not_existing",
				...
				try {
					...
					LogService.getRoot().log(Level.WARNING,
							I18N.getMessage(LogService.getRoot().getResourceBundle(),
									"base.operators.tools.plugin.Plugin.operator_descriptor_reading_error", operatorsURL,
									archive.getName()),
							e);
					return;
				}
			...
			LogService.getRoot().log(Level.INFO, "base.operators.tools.plugin.Plugin.operator_descriptor_not_specified",
		...
		if (in != null) {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.operator_descriptor_not_defined",
					getName());
		}
	}
...
//			}
//		} catch (Exception e) {
//			I18N.getMessage(ResourceBundle.getBundle("base.operators.resources.i18n.LogMessages"),
//					"I18N.plugin_warning1", Level.WARNING, getName(), e);
//		}
...
//			// getName(), e);
//			I18N.getMessage(ResourceBundle.getBundle("base.operators.resources.i18n.LogMessages"),
//					"I18N.plugin_warning2", Level.WARNING, getName(), e);
//		}
...
//	}
	...
	private static void findAndRegisterPlugins(File pluginDir, boolean showWarningForNonPluginJars,
		...
		if (pluginDir == null) {
			LogService.getRoot().log(Level.WARNING,
					"base.operators.tools.plugin.Plugin.findandregisterplugins_called_with_null_directory");
			return;
		}
		if (!(pluginDir.exists() && pluginDir.isDirectory())) {
			LogService.getRoot().log(Level.CONFIG, "base.operators.tools.plugin.Plugin.plugin_dir_not_existing", pluginDir);
		} else {
			LogService.getRoot().log(Level.CONFIG, "base.operators.tools.plugin.Plugin.scanning_for_plugins", pluginDir);
	 ...
	 */
	public static void registerPlugins(List<File> files, boolean showWarningForNonPluginJars,
			boolean overwritePluginsWithHigherVersions) {
		...
		for (File file : files) {
			try (JarFile jarFile = new JarFile(file)) {
				...
				if (RAPIDMINER_TYPE_PLUGIN.equals(attributes.getValue(RAPIDMINER_TYPE))) {
					...
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.plugin.Plugin.jar_file_does_not_contain_entry",
							new Object[]{jarFile.getName(), RAPIDMINER_TYPE});
				}
			} catch (Throwable e) {
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.tools.plugin.Plugin.plugin_loading_error", file, e.getMessage()), e);
			}
		}
	...
	private static void resolveVersionConflict(Plugin newExtension, Plugin conflictingExtension,
		...
		LogService.getRoot().log(Level.WARNING,
				"base.operators.tools.plugin.Plugin.duplicate_plugin_definition_higher_version",
		...
		if (!ALL_PLUGINS.isEmpty()) {
			...
			while (i.hasNext()) {
				...
				try {
					...
					LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.plugin.Plugin.plugin_initializing_error", e), e);
					i.remove();
					plugin.disabled = true;
					INCOMPATIBLE_PLUGINS.add(plugin);
				}
	...
	private static boolean isExtensionVersionAllowed(final Plugin extension, final VersionNumber extensionVersion) {
		...
		if (!allowed) {
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.plugin_studio_core_version_too_high",
	...
	private final boolean isIncompatible() {
		...
		if (PLUGIN_BLACKLIST.containsKey(getExtensionId())) {
			...
			if (forbiddenRange == null) {
				LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.incompatible_extension",
						new Object[] { getName() });
				return true;
			}
			...
			if ((startVersion != null && currentVersion.isAtLeast(startVersion) || startVersion == null)
				...
				if (startVersion != null && endVersion != null) {
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.plugin.Plugin.incompatible_extension_version",
							new Object[] { getName(), startVersion, endVersion, currentVersion });
				} else if (startVersion != null) {
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.plugin.Plugin.incompatible_extension_version_above",
							new Object[] { getName(), startVersion, currentVersion });
				} else if (endVersion != null) {
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.plugin.Plugin.incompatible_extension_version_below",
			...
			while (iterator.hasNext()) {
				...
				for (Dependency dependency : plugin.pluginDependencies) {
					...
					if (dependencyPlugin == null) {
						...
						INCOMPATIBLE_PLUGINS.add(plugin);
						iterator.remove();
						LogService.getRoot().log(Level.SEVERE, "base.operators.tools.plugin.Plugin.loading_extension_error",
...
//	}
	 ...
	 * base.operators.PluginInit for arbitrary initializations of the plugins. It is called directly
	...
	private boolean callInitMethod(String methodName, Class<?>[] arguments, Object[] argumentValues,
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING,
					I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.plugin.Plugin.plugin_initializer_error", pluginInitClassName, methodName,
		...
		if (loadPlugins && !isSafeMode) {
			...
			if (pluginDirString != null && !pluginDirString.trim().isEmpty()) {
				...
				if (!pluginDir.isDirectory() && !pluginDir.mkdirs()) {
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.plugin.Plugin.could_not_create_user_home_extension_directory");
				}
			...
			if (loadFromGlobalFolder()) {
				try {
					...
					LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.no_properties_set",
...
//			updateStudioUpdatePolicy();
			...
			LogService.getRoot().log(Level.INFO, "base.operators.tools.plugin.Plugin.plugins_skipped");
		}
		...
		for (Entry<String, Long> entry : sortedLoadingTimes) {
			...
			String identifier;
			if (plugin != null) {
				LogService.getRoot().log(logLevel, "base.operators.tools.plugin.Plugin.loading_time",
						new Object[] { plugin.getName(), value });
				identifier = plugin.getExtensionId();
			} else {
				LogService.getRoot().log(logLevel, "base.operators.tools.plugin.Plugin.loading_time_failure",
	...
	/**
	 * @return {@code false} if the {@link RapidMiner.ExecutionMode} of RapidMiner is embedded, web
	...
	/**
	 * @return {@code false} if the {@link RapidMiner.ExecutionMode} of RapidMiner is embedded or web
...
//		}
	...
	private void updateSettingsDescriptions() {
		if (settingsRessourceBundle != null) {
			for (String settingsKey : settingsRessourceBundle.keySet()) {
				if (settingsKey.endsWith(I18N.SettingsType.DESCRIPTION.toString())) {

					// Extract key of ParameterType by removing suffix of description
					String parameterTypeKey = settingsKey.substring(0,
							settingsKey.length() - I18N.SettingsType.DESCRIPTION.toString().length());

					ParameterType parameterType = ParameterService.getParameterType(parameterTypeKey);
					if (parameterType != null) {
						String description = I18N.getSettingsMessage(parameterTypeKey, I18N.SettingsType.DESCRIPTION);
	...
	private static File updateWebstartPluginsCache() {
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
					"base.operators.tools.plugin.Plugin.hashing_remote_url_error", e), e);
			return null;
		}
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
					"base.operators.tools.plugin.Plugin.creating_file_error", readmeFile, e1), e1);
		}
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
					"base.operators.tools.plugin.Plugin.loading_extensions_list_error", e), e);
			return null;
		}
		...
		for (int i = 0; i < pluginElements.getLength(); i++) {
			...
			if (pluginFile.exists()) {
				LogService.getRoot().log(Level.CONFIG, "base.operators.tools.plugin.Plugin.extension_found_cache_exists",
						pluginName);
			} else {
				LogService.getRoot().log(Level.CONFIG, "base.operators.tools.plugin.Plugin.extension_found_downloading",
						pluginName);
				try {
					...
					LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.plugin.Plugin.downloading_extension_error", e), e);
		...
		if (!errorOccurred) {
			for (File file : cacheDir.listFiles()) {
				...
				if (!cachedFiles.contains(file)) {
					LogService.getRoot().log(Level.CONFIG, "base.operators.tools.plugin.Plugin.deleting_obsolete_file",
							file);
					file.delete();
				}
			}
	...
	public void reregister() {
		...
		try {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.register_desc_runtime_failed",
					new Object[] { this.getName(), e.getMessage() });
		}
		try {
			registerOperators();
		} catch (Exception e) {
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.plugin.Plugin.register_operators_runtime_failed",
					new Object[] { this.getName(), e.getMessage() });
		}
	}
