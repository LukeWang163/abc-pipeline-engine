...
public class FileSystemService {
	...
	public static File getMainUserConfigFile() {
		return FileSystemService.getUserConfigFile(ParameterService.RAPIDMINER_CONFIG_FILE_NAME);
	}
	...
	public static File getLogFile() {
		return new File(getUserRapidMinerDir(), "operators-studio.log");
	}
	...
	public static File getUserRapidMinerDir() {
		File rapidMinerDir;
		String customHome = System.getProperty("operators.user-home");
	...
	public static File getSourceRoot() {
		String srcName = System.getProperty(ParameterService.PROPERTY_RAPIDMINER_SRC_ROOT);
		if (srcName == null) {
			LogService.getRoot().log(Level.WARNING, "base.operators.tools.FileSystemService.property_not_set",
					ParameterService.PROPERTY_RAPIDMINER_SRC_ROOT);
			return null;
		} else {
	...
	private static void checkAndCreateFolder(File newFolder) {
		if (!newFolder.exists()) {
			LogService.getRoot().log(Level.CONFIG, "base.operators.tools.FileSystemService.creating_directory", newFolder);
			boolean result = newFolder.mkdir();
			if (!result) {
				LogService.getRoot().log(Level.WARNING,
						"base.operators.tools.FileSystemService.creating_home_directory_error", newFolder);
			}
		}
	}
