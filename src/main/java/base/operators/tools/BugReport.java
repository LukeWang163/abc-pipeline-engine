...
public class BugReport {
	...
	private static String getRelevantLogContent() {
		...
		try (FileReader fr = new FileReader(FileSystemService.getLogFile())) {
			...
			LogService.getRoot().log(Level.WARNING, "base.operators.logservice.logfile.failed_to_read", e.getMessage());
			...
			if (reader != null) {
				try {
					...
					LogService.getRoot().log(Level.WARNING, "base.operators.logservice.logfile.failed_to_close",
							e.getMessage());
				}
			}
		}
