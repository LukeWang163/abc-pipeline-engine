		...
		try {
			...
			switch (methodIndex) {
				...
				case RapidMiner.PROPERTY_RAPIDMINER_TOOLS_MAIL_METHOD_SENDMAIL:
					...
					break;
				default:
					LogService.getRoot().log(Level.SEVERE, "base.operators.tools.MailUtilities.illegal_send_mail_method",
							method);
			...
			if (mailSender != null) {
				...
				LogService.getRoot().log(Level.INFO, "base.operators.tools.MailUtilities.sent_mail_to_adress_with_subject",
						new Object[] { address, subject });
			}
		} catch (Exception e) {
			LogService.getRoot().log(Level.SEVERE, "base.operators.tools.MailUtilities.sending_mail_to_address_error",
