...
import base.operators.studio.internal.RuleProviderRegistry;
...
enum RuleService {
	...
	public void reloadRules() {
		if (!RapidMiner.getExecutionMode().equals(RapidMiner.ExecutionMode.UI)) {
			return;
		}
			...
			try (InputStream ruleJson = provider.getRuleJson()) {
				if (ruleJson != null) {
					...
					LogService.getRoot().log(Level.FINE, I18N.getMessage(LogService.getRoot().getResourceBundle(),
							"base.operators.tools.usagestats.RuleService.load.empty", provider.getClass().getSimpleName()));
				}
			} catch (Exception e) {
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.tools.usagestats.RuleService.load.failure", provider.getClass().getSimpleName()), e);
				break;
			}
		}
			...
			for (VerifiableRule rule : loadedRules) {
				...
				if (rule.getRule().getMinStudioVersion() != null) {
					try {
						...
						LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
								"base.operators.tools.usagestats.RuleService.load.invalid_min_version", rule.getRule().getMinStudioVersion()));
						continue;
					}
				}
				...
				if (rule.getRule().getMaxStudioVersion() != null) {
					try {
						...
						// malformed version number in JSON, skip rule
						LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
								"base.operators.tools.usagestats.RuleService.load.invalid_max_version", rule.getRule().getMinStudioVersion()));
					}
				}
			}
		...
		for (Rule rule : jsonRules) {
			...
			if (matcher.find()) {
				...
				LogService.getRoot().log(Level.WARNING,
						"base.operators.tools.usagestats.RuleService.load_invalid_sql",
