	...
	CtaDataSource() {
		try {
			...
			if (e.getErrorCode() == ErrorCode.FILE_CORRUPTED_1) {
				...
				LogService.getRoot().log(Level.WARNING, "base.operators.tools.usagestats.CtaDataSource.database.currupted",
						e);
				DeleteDbFiles.execute(RM_HOME, DATABASE, true);
				// The connection should be hopefully established on the next getConnection() call.
			} else {
				LogService.getRoot().log(Level.WARNING, "base.operators.tools.usagestats.CtaDataSource.open.failed", e);
			}
