...
import base.operators.tools.WebServiceTools;
...
public class UsageStatistics {
	...
	private static final String WEB_SERVICE_URL = "https://stats.operators.base/usage-stats/upload/operators";
	...
	private void load() {
		if (Telemetry.USAGESTATS.isDenied()) {
			LogService.getRoot().log(Level.CONFIG,
					"base.operators.gui.telemetry.accessing_online_services_disallowed");
			return;
		}
		if (!RapidMiner.getExecutionMode().canAccessFilesystem()) {
			LogService.getRoot().log(Level.CONFIG,
					"base.operators.gui.tools.usagestats.UsageStatistics.accessing_file_system_error_bypassing_loading");
			return;
		}
		...
		if (file.exists()) {
			try {
				LogService.getRoot().log(Level.CONFIG,
						"base.operators.gui.tools.usagestats.UsageStatistics.loading_operator_statistics");
				...
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
						"base.operators.gui.tools.usagestats.UsageStatistics.loading_operator_usage_error", e), e);
				init();
			}
		} else {
			init();
	...
	private Document getXML(Reason reason, ActionStatisticsCollector.ActionStatisticsSnapshot snapshot) {
		...
		root.setAttribute("operators-version", new RapidMinerVersion().toString());
	...
	public void save() {
		...
		if (!RapidMiner.getExecutionMode().canAccessFilesystem()) {
			LogService.getRoot().config(
					"base.operators.gui.tools.usagestats.UsageStatistics.accessing_file_system_error_bypassing_save");
			return;
		}
		...
		try {
			LogService.getRoot().log(Level.CONFIG,
					"base.operators.gui.tools.usagestats.UsageStatistics.saving_operator_usage");
			...
			LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(),
					"base.operators.gui.tools.usagestats.UsageStatistics.saving_operator_usage_error", e), e);
		}
	}
