...
public final class PlatformUtilities {
	 ...
	 */
	private static final String RAPIDMINER_STUDIO_LAUNCHER_JAR = "operators-studio-launcher.jar";
	...
	static {
		...
		BUILD_PATH_ELEMENTS.add("operators-studio-core");
		BUILD_PATH_ELEMENTS.add("operators-studio-commons");
	}
	...
	public static final String PROPERTY_RAPIDMINER_HOME = "operators.home";
...
//        return currentVersion;
	...
	/**
	 * @return the system property 'operators.home'.
	 */
	...
	private static String readResourceProperty(String propertyKey) {
		...
		try (InputStream resourceAsStream = PlatformUtilities.class.getResourceAsStream("version.properties")) {
			if (resourceAsStream == null) {
				logInfo("Version resource file not found at 'base/operators/tools/version.properties'.");
				return null;
			} else {
	...
	/**
	 * Ensures that the environment variable 'operators.home' is set by calling
	...
	/**
	 * Ensures that the environment variable 'operators.home' is set by searching for RapidMiner
		...
		if (getRapidMinerHome() == null) {
			...
			if (!searchInClassPath()) {
				try {
					...
					logSevere("Failed to locate 'operators.home'! Cause: " + e.getLocalizedMessage());
				}
			}
		} else {
	...
	/**
	 * Checks the Java Classpath for operators studio launcher or core jars.
	 *
	 * @return
	 */
	private static boolean searchInClassPath() {
		...
		for (int i = 0; i < pathComponents.length; i++) {
			...
			/*
			 * Search for either 'operators-studio-launcher.jar' or for
			 * 'operators-studio-core-x.x.xxx.jar'
			 */
			if (path.matches(RAPIDMINER_STUDIO_CORE_REGEX) || path.endsWith(RAPIDMINER_STUDIO_LAUNCHER_JAR)) {
				...
				if (dir == null) {
					logSevere("Failed to retrieve 'operators.home'. Parent of jar is not a directory!");
				} else {
					dir = retrieveRMHomeFromLibraryDir(dir);
					if (dir != null && dir.isDirectory()) {
						logInfo("Gotcha! 'operators.home' is: " + dir);

						System.setProperty(PROPERTY_RAPIDMINER_HOME, dir.getAbsolutePath());
						return true;
					} else {
						logSevere("Failed to retrieve 'operators.home'. Parent of jar directory is not a directory!");
					}
				}
			}
		}
	...
	/**
	 * Searches for 'operators.home' assuming a development environment. Start looking for
	 * 'operators-studio' folder.
	 */
	private static void searchInBuildDir() {
		...
		if (url == null) {
			logSevere("Failed to locate 'operators.home'. Could not locate base directory of classes!");
		} else {
			...
			if (buildDir != null && buildDir.isDirectory()) {
				...
				if (rmHome != null && rmHome.isDirectory()) {
					...
					logInfo("Gotcha! operators.home is: " + rmHome);
					try {
					...
					logSevere("Failed to locate 'operators.home'! Parent dir of build directory does not exist.");
				}
			} else {
				logSevere("Failed to locate 'operators.home'! Build directory does not exists or isn't a directory.");
			}
		}
	}
