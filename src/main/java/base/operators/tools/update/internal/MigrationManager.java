 ...
 */
package base.operators.tools.update.internal;
...
public final class MigrationManager {
	...
	static class MigrationStep {
		...
		private void runSafe() {
			try {
				LogService.getRoot().log(Level.INFO, "base.operators.tools.update.internal.MigrationManager.starting", name);
				runnable.run();
				LogService.getRoot().log(Level.INFO, "base.operators.tools.update.internal.MigrationManager.success", name);

			} catch (Exception e) {
				LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(), "base.operators.tools.update.internal.MigrationManager.failure", name), e);
			}
		}
		...
		if (RapidMiner.getExecutionMode().canAccessFilesystem() && !RapidMiner.getExecutionMode().isHeadless()) {
			...
			if (userConfigFile != null) {
				...
				try (InputStream in = new FileInputStream(userConfigFile)) {
					properties.load(in);
					final String disconnectOnDisableKey = "operators.gui.disconnect_on_disable";
