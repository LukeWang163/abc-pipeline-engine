 ...
 */
package base.operators.tools.abtesting;
...
final class IdentifierProvider {
	...
	private static String getIdentifierString() {
		try {
			...
			LogService.getRoot().log(Level.FINEST, "base.operators.tools.abtesting.IdentifierProvider.email_retrieval_failed", e);
		}

		try {
			return getUserAndOSVersion();
		} catch (Exception e) {
			LogService.getRoot().log(Level.FINEST, "base.operators.tools.abtesting.IdentifierProvider.system_properties_retrieval_failed", e);
		}

		try {
			return getHardwareAddress();
		} catch (Exception e) {
			LogService.getRoot().log(Level.FINEST, "base.operators.tools.abtesting.IdentifierProvider.mac_address_retrieval_failed", e);
		}
