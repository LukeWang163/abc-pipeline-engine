...
import base.operators.tools.cipher.CipherException;
import base.operators.tools.cipher.CipherTools;
	...
	public Session makeSession() {
		...
		if (host == null) {
			LogService.getRoot().log(Level.WARNING,
					"base.operators.tools.DefaultMailSessionFactory.smtp_host_must_be_specified",
			...
			if (from != null && from.contains("@")) {
				...
				props.put("mail.from", "no-reply@operators.base");
			}
			...
			try {
				if (CipherTools.isKeyAvailable()) {
					...
					passwd = "";
					LogService.getRoot().log(Level.WARNING,
							"base.operators.tools.DefaultMailSessionFactory.smtp_password_cipher_missing");
				}
				if (passwd.length() > 0) {
				...
				LogService.getRoot().log(Level.WARNING,
						"base.operators.tools.DefaultMailSessionFactory.smtp_password_decode_failed");
			}
