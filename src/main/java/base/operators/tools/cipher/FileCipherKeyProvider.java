...
*/
package base.operators.tools.cipher;

import base.operators.RapidMiner;
import base.operators.io.Base64;
...
//			LogService.getRoot().log(logLevel, "base.operators.tools.cipher.KeyGeneratorTool.read_key_error",
//					e.getMessage());
...
//		}
		...
		SecretKey key = null;
		try {
			key = KeyGeneratorTool.makeKey(Base64.decode("wfIEBGjv1YraVBPfdu+Mm6dK8f66jHb4"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		// try to load key from file
		File keyFile = new File("C:\\Users\\zhanglsh01\\.RapidMiner\\cipher.key");
		try (FileInputStream fis = new FileInputStream(keyFile); ObjectInputStream in = new ObjectInputStream(fis)) {
			int length = in.readInt();
			byte[] rawKey = new byte[length];
			int actualLength = in.read(rawKey);
			...
			SecretKey key = KeyGeneratorTool.makeKey(rawKey);
