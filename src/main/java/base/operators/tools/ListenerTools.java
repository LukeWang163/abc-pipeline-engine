...
import static base.operators.tools.ConsumerWithThrowable.wrapAndReturn;
...
public final class ListenerTools {
	...
	private static final Consumer<Throwable> THROWABLE_LOGGER = t -> LogService.getRoot().log(Level.WARNING, "base.operators.tools.ListenerTools.uncaught_exception", t);
	...
	public static <L, E extends Throwable> List<Throwable> informAllAndThrow(Collection<L> listeners, ConsumerWithThrowable<L, E> method, Class<E> eClass) throws E {
		Map<Boolean, List<Throwable>> errorMap = listeners.stream().map(ConsumerWithThrowable.wrapAndReturn(method))
