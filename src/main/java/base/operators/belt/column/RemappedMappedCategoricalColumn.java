...
class RemappedMappedCategoricalColumn<R> extends CategoricalColumn<R> implements CacheMappedColumn {
	...
	private Column deriveMappedColumn(int[] mapping) {
		return getRemappedMappedCategoricalColumn(dictionary, remapping, mapping);
	}
	...
	private int lookUpIndexUInt2(int i) {
		...
		if (position >= 0 && position < byteData.size()) {
			return remapping[readUInt2(byteData.data(), position)];
		} else {
			return 0;
		}
	}
	...
	private int lookUpIndexUInt4(int i) {
		int position = mapping[i];
		if (position >= 0 && position < byteData.size()) {
			return remapping[readUInt4(byteData.data(), position)];
		} else {
			return 0;
		}
	}
	...
	private int lookUpIndexUInt8(int i) {
		int position = mapping[i];
		if (position >= 0 && position < byteData.size()) {
			return remapping[Byte.toUnsignedInt(byteData.data()[position])];
		} else {
			return 0;
		}
	}
	...
	private int lookUpIndexUInt16(int i) {
		...
		if (position >= 0 && position < shortData.length) {
			return remapping[Short.toUnsignedInt(shortData[position])];
		} else {
			return 0;
		}
	}
	...
	private int lookUpIndexInt32(int i) {
		...
		if (position >= 0 && position < intData.length) {
			return remapping[intData[position]];
		} else {
			return 0;
		}
	}
