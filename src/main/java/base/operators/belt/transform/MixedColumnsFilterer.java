...
/**
 * Filters by several {@link Column}s using a given filter operator.
 *
 * @author Gisa Meier
 */
final class MixedColumnsFilterer implements Calculator<int[]> {

	private final List<Column> sources;
	private final Predicate<MixedRow> operator;
	private boolean[] target;
	private AtomicInteger found = new AtomicInteger();

	MixedColumnsFilterer(List<Column> sources, Predicate<MixedRow> operator) {
	...
	/**
	 * Writes the result of the filter into the target array for every index between from (inclusive) and to
	 * (exclusive) of the source column using the operator.
	 */
	private static int filterPart(List<Column> sources, boolean[] target, Predicate<MixedRow> operator, int from,
                                  int to) {
		int found = 0;
