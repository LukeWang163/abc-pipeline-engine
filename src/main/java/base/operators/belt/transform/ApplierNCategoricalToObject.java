...
/**
 * Maps {@link Column.Category#CATEGORICAL} {@link Column}s to a {@link ObjectBuffer} using a given mapping
 * operator.
 *
 * @author Gisa Meier
 */
final class ApplierNCategoricalToObject<T> implements Calculator<ObjectBuffer<T>> {
	
	private ObjectBuffer<T> target;
	private final List<Column> sources;
	private final Function<CategoricalRow, T> operator;

	ApplierNCategoricalToObject(List<Column> sources, Function<CategoricalRow, T> operator) {
	 ...
	 */
	private static <T> void mapPart(final List<Column> sources, final Function<CategoricalRow, T> operator,
                                    final ObjectBuffer<T> target, int from, int to) {
