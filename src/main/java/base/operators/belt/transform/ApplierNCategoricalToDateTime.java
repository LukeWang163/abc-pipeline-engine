...
/**
 * Maps {@link Column.Category#CATEGORICAL} {@link Column}s to a {@link DateTimeBuffer} using a given
 * mapping operator.
 *
 * @author Gisa Meier
 */
final class ApplierNCategoricalToDateTime implements Calculator<DateTimeBuffer> {

	private DateTimeBuffer target;
	private final List<Column> sources;
	private final Function<CategoricalRow, Instant> operator;

	ApplierNCategoricalToDateTime(List<Column> sources, Function<CategoricalRow, Instant> operator) {
	 ...
	 */
	private static void mapPart(final List<Column> sources, final Function<CategoricalRow, Instant> operator,
