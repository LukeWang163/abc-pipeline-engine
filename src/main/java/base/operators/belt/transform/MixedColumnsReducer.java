...
/**
 * Reduces several arbitrary {@link Column}s using a given reduction information.
 *
 * @author Gisa Meier
 */
final class MixedColumnsReducer<T> implements Calculator<T> {


	private final List<Column> sources;
	private final Supplier<T> supplier;
	private final BiConsumer<T, MixedRow> reducer;
	private final BiConsumer<T, T> combiner;
	private NumericColumnReducer.CombineTree<T> combineTree;

	MixedColumnsReducer(List<Column> sources, Supplier<T> supplier, BiConsumer<T, MixedRow> reducer, BiConsumer<T, T> combiner) {
	 ...
	 */
	private static <T> void reducePart(List<Column> sources, T container, BiConsumer<T, MixedRow> reducer, int from, int to) {
