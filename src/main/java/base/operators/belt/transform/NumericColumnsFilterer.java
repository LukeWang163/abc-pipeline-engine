...
/**
 * Filters by several {@link Column.Capability#NUMERIC_READABLE} {@link Column}s using a given filter operator.
 *
 * @author Gisa Meier
 */
final class NumericColumnsFilterer implements Calculator<int[]> {
	...
	NumericColumnsFilterer(List<Column> sources, Predicate<NumericRow> operator) {
	 ...
	 */
	private static int filterPart(List<Column> sources, boolean[] target, Predicate<NumericRow> operator, int from, int to) {
		int found = 0;
